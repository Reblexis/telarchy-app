...
vi.mock('../../lib/api', () => {
  const explicit: Record<string, unknown> = {
    ...
    getProfile: vi.fn(async () => ({ workspaceId: 'ws-1', capabilities: ['read', 'trade', 'manage'] })),
...
describe('"Manage dates" at the end of the dates strip', () => {
  test('opens the metrics dialog straight onto the sheet of the metric on screen', async () => {
    renderFloor();
    // The way in is the last tab of the dates strip (docs/ui-conventions.md,
    // "The question line"); it is there once the page knows the viewer can
    // manage.
    const manage = await screen.findByText('Manage dates');
