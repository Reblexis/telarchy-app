...
export function ConnectorsPage() {
  ...
  const handleExpand = async (connectorId: string) => {
    ...
    setFileContent(null);
    setFilePath(null);
    await loadTree(connectorId, '');
  };

  const loadTree = async (connectorId: string, path: string) => {
    setLoadingTree(true);
    setFileContent(null);
    setFilePath(null);
    try {
      const entries = await api.getConnectorTree(connectorId, path || undefined) as GitHubTreeEntry[];
  ...
  const handleOpenFile = async (path: string) => {
    ...
    setFilePath(path);
    try {
      const data = await api.getConnectorFile(expandedId, path) as { path: string; content: string };
  ...
  const handleDelete = async (connectorId: string) => {
    if (!confirm('Disconnect this connector? This cannot be undone.')) return;
    setError('');
    try {
    ...
    <div className="container">
      ...
      <div className="section">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
          <div>
            <h2 style={{ fontSize: '1.1rem', fontWeight: 600, marginBottom: '0.25rem' }}>Connectors</h2>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem', margin: 0 }}>
              Connect external data sources (e.g. GitHub repos) for workspace participants to browse.
            </p>
          </div>
          {isAdmin && (
        ...
        {loading ? (
          ...
          <p style={{ color: 'var(--text-secondary)' }}>No connectors yet.{isAdmin ? ' Connect a GitHub repo to get started.' : ''}</p>
        ) : (
