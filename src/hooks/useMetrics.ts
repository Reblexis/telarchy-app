...
export function useMetrics(authenticated: boolean, inspectTaskId?: string | null) {
  ...
  const addMetric = async (
    name: string, description: string, question: string, value: number, formula: string,
    marketRangeMax?: number, checkInIntervalDays?: number,
  ): Promise<string[]> => {
    if (detectCircularDependency(null, formula, metrics)) {
      throw new Error('This formula would create a circular dependency');
    }
    const { id, warnings = [] } = await api.createMetric({ name, description, question, value, formula, marketRangeMax, checkInIntervalDays });
    const updated = [...metrics.map(m => ({ ...m })), { id, name, description, question, value, total: value, formula, order: 999, depth: 0, marketRangeMax, checkInIntervalDays }];
  ...
  const editMetric = async (
    ...
    const updated = metrics.map(m =>
      m.id === id
        ? { ...m, name, description, question, value, formula, marketRangeMax, checkInIntervalDays: checkInIntervalDays ?? m.checkInIntervalDays, timePreference: timePreference === null ? undefined : (timePreference ?? m.timePreference) }
        : { ...m }
    );
    setMetrics(enrichMetrics(updated, consensusMapRef.current));
    setFormulaWarnings(buildWarnings(updated));
    return api.updateMetric(id, { name, description, question, value, formula, oldValue, updateNote, timePreference: timePreference === undefined ? undefined : timePreference, marketRangeMax, checkInIntervalDays })
