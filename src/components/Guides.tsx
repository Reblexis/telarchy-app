...
/**
 * The guides as components: the index and one guide, rendered from what the
 * API serves at /api/guides. Two pages draw them: telarchy.com/guides
 * (pages/GuidesPage.tsx) and the data room's Documentation tab.
 *
 ...
 *
 * The same two components render the data room's Documentation tab
 * (docs/data-room.md, "Documentation") with `base="/data-room/docs"` and
 * `inRoom`: the room supplies its own masthead, tab row and palette, and the
 * links between guides stay under the room's address.
 */

/** Where one guide lives under a given index address. */
const guideHref = (base: string, id: string) => `${base}/${id}`;
...
export function OneGuide({
  section,
  base = '/guides',
  inRoom = false,
}: {
  section: string;
  /** The index's address; every guide-to-guide link is built under it. */
  base?: string;
  /** Inside the data room: no page chrome of its own. */
  inRoom?: boolean;
}) {
  ...
  const Missing = inRoom ? 'h2' : 'h1';
  const content = (
    <>
      <p className="pubws-h2">
        <Link to={base}>All guides</Link>
      </p>
      {failed ? (
        <>
          <Missing className="pubws-name">That guide is not here</Missing>
          <p className="pubws-pitch">
            The address does not match a guide. <Link to={base}>The index</Link> lists every one of them.
          </p>
        </>
      ) : (
            ...
            <ReactMarkdown
              ...
              components={{
                a: ({ href, children }) => {
                  ...
                  const target = guide ? `${guideHref(base, guide[1])}${guide[2] ?? ''}` : local;
...
export function GuideIndex({ base = '/guides', inRoom = false }: { base?: string; inRoom?: boolean }) {
        ...
        byCategory.map(group => (
          <section className="pubws-section" key={group.category}>
            ...
            <ul className="pubws-contact">
              {group.items.map(s => (
                <li key={s.id}>
                  <Link to={guideHref(base, s.id)} className="pubws-guide-link">
                    {s.title}
                  </Link>
