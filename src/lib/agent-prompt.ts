...
export function agentPrompt(origin: string, floor: FloorRef | null): string {
  if (floor) {
    ...
    return [
      `You are researching ${floor.name} on Telarchy, where a market prices what each proposed proposal would do to the company's real numbers.`,
      '',
      `1. Read the brief: GET ${base}/context?format=md`,
      "   It carries the company, every metric with its history, the open markets and their current prices, every proposal with the market's priced impact, and the owner's published documents. Drop ?format=md for JSON.",
...
export function traderAgentPrompt(
  ...
  return [
    ...
    '  It carries the company, every metric with its history, the open markets and their prices, and every proposal with what the market says it would do to the number.',
    '',
    'HOW TO ACT AS ME',
    ...grantLine(grant, workspaceId ?? '<the workspace id from the brief>'),
    `- The endpoint catalog is GET ${origin}/api/help. Trading, limit orders and proposals are all in it.`,
    '',
    'THE CALLS',
