...
describe('announcementHeadline', () => {
  ...
  test('a long opening paragraph is cut to its first sentence', () => {
    ...
    const body = 'Season 0 starts Friday 00:00 UTC. It runs to 16 October, the pool is $1,000 of real money paid $500 / $250 / $125 / $75 / $50 to the top five.\n\nEntry is one click.';
    expect(announcementHeadline(body)).toBe('Season 0 starts Friday 00:00 UTC.');
  });

  test('markdown furniture never reaches the floor', () => {
    expect(announcementHeadline('## **Season 0** is [live](https://telarchy.com/season)'))
      .toBe('Season 0 is live');
  });

  test('a first sentence with no end still gets cut on a word boundary', () => {
    const body = 'We are moving the September market to a new range because the old one capped at a number the metric has already passed twice this month';
