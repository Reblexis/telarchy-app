...
The new-bot form asks only an optional name and starting credits; it has no identity selector, because personal keys are created in their own section with a permission preset. Creation closes the form and shows the new bot as the first card, with its API key shown once above the card’s actions. Runtime setup belongs with the bot, not the creation form. **Copy setup prompt** is available beside the created connection and in each key’s runtime controls. **Set up manually** sits beside it and opens an inline alternative using the same identity and permission choices. The manual path is three steps: preview, create the key, connect the key. The preview block clones the deterministic reference agent, installs it and runs it; it needs no account, sets no variables and spends nothing. Connecting is one command, `agent.py --login`: the agent asks for the key with a hidden prompt, checks it, and saves it on the visitor's computer, so the page never pastes a key prompt together with other lines (a hidden prompt reads the lines pasted after it as the key). A second one-line preview then shows the estimated price of each trade. A visitor who chose research access is told they can stop after the first preview. Trading commands (`--live`, and `--live --every 30` to keep running) are disclosed only when trading access is selected. Initial commands never trade, regardless of the selected access. macOS/Linux and Windows PowerShell have separate runnable commands. Commands never embed actual credentials. With no chosen workspace the commands name none and the agent uses the public Telarchy workspace, which the page says; a chosen workspace rides on every run as a shell-quoted `--workspace` flag. Every file and flag the page or the build guide names must exist on the reference agent's main branch; `scripts/check-reference-agent-commands.mjs` enforces it in CI. The build guide supplies the AI-assisted alternative. Manual setup and prompt handoff preserve the same unfinished connection state. When a key already exists, manual setup skips the create step and offers to copy that same key under the connect command, for when the terminal asks for it. Public previews remain available through the runtime instructions without creating a key. Trading upgrades and extra funding are secondary disclosures after a key is created.
...
The page is two columns from 900px, Bots on the left and Your API keys on the right, stacking to one column below that. Bots are a table of hairline rows, not cards: one caption row (Bot, Credits, Earned, Trades) and under it a row per bot with the bot mark and its name linking to its participant profile, then the three numbers right-aligned in mono, so every bot an owner has reads down aligned columns and a screen holds them all. A bot that has never traded reads "no trades yet" in the Earned column rather than a confident zero. Below 520px the caption row goes and each row carries its own three captioned cells under the name.

A row opens and closes on a press anywhere on it except the name (its toggle is a button named "Actions for <bot>"), one row open at a time. The open row shows the bot's actions and the day of its last trade when it has one ("last trade 2026-09-17"): **Copy setup prompt** first, the one outlined button (it copies at once and confirms in place; the prompt uses the bot's identity and full access without naming a workspace), then as plain text buttons **Set up manually**, **Send credits** (an inline form naming the source balance), and **Keys**, which opens the bot's keys inside the row with Edit, Revoke and New key. No bot row carries a filled button: the page's one filled button is the new-bot form's. Nothing is named Set up or Manage. On arrival the open row is the bot created in this session, which stands first with its key shown once above the actions; otherwise an owner's only bot is open, and with several bots every row starts closed. With no bots, the column is a compact block that says a bot has its own balance and record and offers Create your first bot. The page states that agents run on the user's computer or server; Telarchy manages access. No status claims that a bot is online: trading history is the available evidence.

The new-bot form is one bar: Name, Starting credits and the create button on a single line from 520px, Cancel beside the form's heading, and one helper line under the credits field, the owner's balance. A second sentence appears only when it says something the fields do not: "Starts with 0 credits. You can fund it later.", or that the balance is too small.

The Your API keys column lists personal keys as hairline rows of one block each: the label on the first line, the access chip and "Not used yet" or "Last used <date>" in mono on the second, and the Run, Edit and Revoke actions as plain text buttons at the right of the block, centred on it; the chip carries the plain-language meaning as its hover title and no sentence repeats it. Run opens the runtime handoff for that key inline. New key opens an inline form with a label and the permission preset, and the created key is shown once in its row.
...
The form carries no link to a feed's own bot guide, from any door: a fed
market's guide is linked from that market's about text, and the form stays
the same form for every market.
