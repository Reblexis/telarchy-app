...
**Verified on beta, 2026-08-23.** A three-turn conversation about Kleros
produced a prompt that opens with the checklist call, names the subgraph, the
ceiling and the horizon, gives the exact `POST /api/metrics` payload, encodes
the key order (the agent registers itself, keeps its key, asks to be added as
admin), lists the six decisions still open, and tells the agent to ask rather
than guess and to confirm before spending. `settled` came back as floor,
number, updates. That is the claim this whole thing makes, and it holds.
