...
- **Proposal economics** (`PUT /api/workspaces/:id/settings`): `proposalReward` pays proposers on approval, `spamPenalty` charges bad-faith ones, `maxPendingProposalsPerParticipant` caps throughput. `notificationsMuted: true` switches off every email, push and bell notification about the workspace, for a floor that decides too many proposals to announce each one. `externalProposalsDisabled: true` lets only the owner and their admins post proposals, for a floor whose proposals are the owner's own. Leave them at defaults for a first workspace.
