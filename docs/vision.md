...
This is the post-USDC-settlement state of the marketplace. On the managed instance (telarchy.com) credits are play-money today, so the alignment described below operates symbolically (calibration leaderboard, reputation) but not in real dollars. On self-hosted instances with `USDC_SETTLEMENT_ENABLED=true`, every flow described below settles in USDC at the configured `creditValueUsd`. The frame is the same in both modes; only the unit changes. When the managed instance turns on settlement (regulatory posture pending), the play-money mode collapses into the real-money mode without any change to the mechanism or framing.
