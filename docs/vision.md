...
2. The system creates **dual-branch conditional markets**. The pair is spawned when the proposal is posted (a subsidy that cannot be covered fails the post with 400); a fetch with `?proposalId=` re-spawns it only for a pending proposal that has no live markets. Each branch prices the difference from the baseline market's forecast, on a range half the metric's either side of zero, and opens at zero; the approved branch of a paid job opens at minus its ask on metrics that burn dollars (docs/guides/creating.md, "A conditional pair prices the difference from the baseline"). For every active leaf-metric market, two books spawn under the proposal, one with `branch="approved"` (priced under "how much will metric X change if this proposal is approved?") and one with `branch="declined"` (the same, if declined). The baseline moving moves neither; deciding records the baseline's forecast as the reference the surviving branch settles against. Pairs from before this rule that anyone had traded keep pricing the level (docs/market-integrity.md I1c).
