...
- **`/manage` is a door to a conversation, not a creation wizard.** It is the owner side's surface: a pitch and a waitlist signup. The first operators are set up by a human, which is what the concierge programme (private notes) assumes. A form that asks for a name, a number and a ceiling would be the console's create-workspace flow with better typography, and the operator experience is designed rather than restored. What Telarchy offers an operator and what setting one up consists of is the operator-door design note (private notes); the screen follows that answer, not the other way round.
