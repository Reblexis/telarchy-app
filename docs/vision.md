...
- **Workspace creation is open at the API.** `POST /api/workspaces` is open to any identity: a browser session or a participant key, no invite. Two brakes apply to callers who are not platform admins, both about the shopfront rather than about trust: three workspaces per account, and `public` clamped to `unlisted` at creation, so a new floor is live, joinable and tradeable by link but is not listed on telarchy.com until a human lists it. The clamp is not tidiness. A running prize season scores over every public workspace (`docs/seasons.md`), so self-serve listing would let someone open a floor, subsidise it out of signup grants and extract that subsidy into an entered account; listing stays a human decision until that hole is closed. `POST /api/onboard`, the unauthenticated one-call variant, is paused: it mints an identity and a workspace together, so there is no account for the cap to count.
...
2. **The owner decides where the liquidity goes.** Specified in `liquidity.md`: a per-workspace liquidity budget, a spread action and per-metric weights over the existing injection primitives. Liquidity is the owner's steering wheel (see "Decision quality scales with capital"): a pool is how an owner says which question is worth answering well.
3. **Credits can be bought.** Resolved without giving credits cash value: an owner buys a funding package (`liquidity.md`), which places credits in the workspace's liquidity budget at 1,000 per dollar and funds a cash prize pool for that workspace's next month (`workspace-pools.md`), paid by Telarchy to traders by settled profit. Credits stay play money and are never redeemed; the money flows one way, owner to Telarchy to trader, which is a free-entry skill contest and needs no gambling or derivatives licence (record: the telarchy umbrella's `notes/real-money-economy-design-2026-08-26.md`). The USDC deposit path stays for self-hosted instances with settlement enabled.

Sequencing: (1) is the only one that blocks a waiting operator, and it is a permission plus one screen. (2) is what makes an operator's second week worth anything, because without it their liquidity sits spread evenly across markets they do not care about. (3) is specified and ships behind `FUNDING_ENABLED`, switched on when Season 0 ends.
