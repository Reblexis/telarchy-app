...
- **Workspace creation is open at the API, and a new floor is public.** `POST /api/workspaces` is open to any identity: a browser session or a participant key, no invite. One brake applies to callers who are not platform admins: three workspaces per account. A new floor defaults to `public` and lands on the telarchy.com list with everyone else (owner decision 2026-08-28: everything public by default for now); an explicit `unlisted` or `private` is honoured, and a signed-in owner's not-yet-public floors still appear to them, first in the grid, badged rather than hidden. ACCEPTED RISK, stated so it is not forgotten: a running prize season scores over every public workspace (`docs/seasons.md`), so self-serve listing lets someone open a floor, subsidise it out of signup grants and extract that subsidy into an entered account. The 2026-08-21 unlisted clamp existed to close that; retiring it reopens it, knowingly, while Telarchy is small enough to watch by hand. `POST /api/onboard`, the unauthenticated one-call variant, is paused: it mints an identity and a workspace together, so there is no account for the cap to count.
