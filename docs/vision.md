...
- **Workspace creation is open at the API, and publishing is one gated step.** `POST /api/workspaces` is open to any identity: a browser session or a participant key, no invite. One brake applies to callers who are not platform admins: three workspaces per account. A new floor defaults to `unlisted`: visible to its owner (first in the home grid, badged "Yours · not public yet"), readable at its link, and one Publish button away from the telarchy.com list. Nothing trades on it until it is public, its owner included: a floor traded in private and published at the end of a season would contribute a month of score at once with nobody having watched any of it. The flip to `public` is refused while the floor has no metric (owner asks 2026-08-28: a visible publish button, and at least one metric to be publishable), because an empty shopfront on the front list serves nobody; an explicit visibility at creation is honoured for API and template callers. ACCEPTED RISK, stated so it is not forgotten: a running prize season scores over every public workspace (`docs/seasons.md`), so self-serve publishing lets someone open a floor, subsidise it out of signup grants and extract that subsidy into an entered account; the metric gate raises the effort without closing the hole, knowingly, while Telarchy is small enough to watch by hand. `POST /api/onboard`, the unauthenticated one-call variant, is paused: it mints an identity and a workspace together, so there is no account for the cap to count.
- **There is no owner door, because the floor is the owner's surface.** Creating one is a dialog on the home page: any signed-in visitor names a floor and lands on it. Everything after that happens on the floor itself, including Otto, who is the same conversation with the same hands wherever he is reached from (owner decision 2026-09-01: management happens from the workspace, and a separate `/manage` link should not exist). The decisions an owner still has open are the governing list in `functions/src/lib/setup-spec.ts`, their state is read from rows (`GET /api/setup/checklist`), and the floor renders the open ones for its owner (`docs/owner-on-the-floor.md`). `/manage` redirects to the home page, because links to it are already printed in other people's notes. Anyone who wants a human has `/contact`.
...
**A conditional market is the proposer's to fund, and the owner's only where they said so.** A proposal may name no `liquiditySubsidy`, and a market at zero liquidity has no price at all: it charts as nothing and the server refuses every trade against it. Funding falls through: the proposal's named contributors first, then, per date, the "Proposal opens with" number the owner set on that date (`timePreference.horizonCredits[entry].proposal`, debited from the workspace owner as baseline markets are, and when the owner cannot cover the full amount, the same share of every branch down to the minimum contribution). That number defaults to 0 (owner decision 2026-09-04, design record in the telarchy umbrella, `notes/proposal-liquidity-per-metric-2026-09-04.md`): a proposal is the proposer's to price, and an owner who auto-funded every proposal watched credits disappear from their account for questions they never asked. The workspace's `autoFundNewMarkets` never covers a proposal. When nobody pays, the markets spawn unfunded and the floor says so in place of the bet buttons, with Inject beside it for a manager, rather than offering a bet the server must reject.
...
For the AI vendor world specifically, this is the substrate the industry is reaching for under the "outcome-based pricing" label. Telarchy provides the verifiable predictor that outcome proposals need; the same mechanism happens to also reward forecasters and proposers in the same outcome-aligned way.
...
| Get a vendor proposal in place | Sign and return the $5,000 vendor proposal |
...
1. **Approval is the payment.** Approving a paid proposal is the payment event: at the press the
   agreed amount is owed and the proposal counts as paid (it is what `approvedUsd` sums, and nothing
...
- **Market administration** is API-only: create, void, resolve and refresh are calls under `/api/predictions`, settling a metric early is `POST /api/metrics/:id/settle` (`market-integrity.md`, "The answer can arrive before the period ends"), and there is no browser screen for any of them.
...
- `comment` (web on, email on, mobile on): someone commented under a proposal you posted, its conditional markets included.
- `reply` (web on, email on, mobile on): someone else commented in a thread you have commented in, proposal or market, and only after you first spoke in it.
...
- `decision` (web on, email on, mobile on): a proposal you posted, traded on (either branch), or commented under anywhere in its conversation was approved or declined. The proposer's own email stays switchless (below); the cells govern everyone else with money or words on the outcome, and the owner who made the decision is never told about their own act.
- `contract` (web on, email off, mobile off): a new proposal went on the ballot in a workspace you belong to. Web on because the bell has always shown these.
...
**A decision on your own proposal has no switch, and always sends.** Approve, decline, and decline-as-spam each mail the proposer the moment the owner decides: which way it went, the ask that was on it, and the written reason when there is one. Every other email here is news about somebody else's activity, which a person is entitled to tune. A decision is not news, it is the answer to the question they asked by posting the proposal, usually with money on it, so the only way that switch would ever be off is a mis-click. The same event is already the `decision` row in the bell; the mail exists because someone who filed a job and closed the tab has nothing else to bring them back.

**A workspace can mute everything it would send.** `notificationsMuted` (`PUT /api/workspaces/:id/settings { notificationsMuted: boolean }`, lifecycle-shaped like visibility, so it needs `manage_workspace`; false by default, and every existing workspace stays unmuted) switches off every notification about that workspace on every channel: no email, no browser push, no bell-inbox row, and no owner mail about a new proposal there. It overrides the switchless decision email above and every participant preference, because it is the workspace owner's decision about their own floor rather than a reader's preference: a floor that decides thousands of proposals a day (a game steered by its market posts four a minute and decides each a minute later) would otherwise mail its proposer, its members and the platform owner about each one. The check sits at the single choke point in `services/notifications.ts`, so a new notification kind is muted with the rest. Unmuting resumes future notifications only; nothing skipped while muted is sent later (the bell, being derived from the record rather than sent, shows the record again once unmuted). The mute is announced nowhere on the floor: it appears only in the workspace payload the owner reads (`GET /api/workspaces/:id`) and in `/api/help`.

Two neighbouring events stay silent on purpose. **Withdrawing** your own proposal is your own doing. **Removing** one from the board is admin cleanup for rows that should never have been there (spam, duplicates, test entries), and it is not a decision, so it produces no decision record: the row stays with status `removed` only so ledger entries keep resolving, is hidden from every listing unless asked for by status, and sends no mail. If the person deserves to hear an answer, decline it with a reason instead of removing it.
...
- Nobody is notified of their own comment or their own proposal, and each recipient gets **at most one email per comment** however many switches would fire. `anyComment` is claimed last, so a watcher who is also the proposal's poster gets the poster's reason and one message rather than two. Two emails for one comment is how a person turns the whole thing off.
- A comment on a **conditional market** counts as a comment on the proposal that market belongs to, so the poster is notified and the email is titled by the proposal rather than the branch. A base market has no poster, so only its thread hears about it.
...
Transport is the shared Resend path in `lib/notify.ts` (`sendEmail`), the same one owner notifications use. With `RESEND_API_KEY` unset nothing is sent at all, which is what local dev and the test suite run on, so no test can mail a real person. Separately from participant notifications, `OWNER_NOTIFY_EMAIL` (when set) receives a mail for every new proposal and every waitlist entry; it has no switch of its own, and only a muted workspace (above) keeps its proposals out of it.
...
A visitor looking at "the market says 25" cannot tell whether 25 is right. What would tell them is spread across a chart, a metric definition, a charter, eight proposals and whatever the owner has written about the business, and reading all of it costs more than the bet is worth. That is the friction that keeps a floor's traffic from becoming its traders, so the floor answers questions.
...
**The markdown brief is ordered for a decision, not for storage.** Proposals still open for a decision come first, under a heading that says so, largest live impact first; decided ones follow with their outcome on the heading. Within a proposal the live horizons come first, and settled ones are collapsed into one line. The JSON keeps every field for code; the markdown is what a model reads, and a model reads the top.
...
**Otto is handed an index and fetches the rest.** The brief is the right payload for an outside agent, which reads it once and does its own work; it is the wrong system prompt for an assistant, because a reasoner handed every number already flattened onto one page will answer from the page instead of looking, and it did (`notes/otto-brief-misread-2026-08-31.md`: zero tool calls on the answer that got four things wrong, five of the previous thirty answers using any tool at all). So his fixed context is the floor's identity, its charter, its metrics with their definitions and readings, its open markets one line each, the owner's announcements and published documents, and then a LIST of proposals carrying title, ask, status and id and no prices at all. A question about a proposal is a question he has to go and answer: `GET /api/marketplace/:idOrSlug/contracts` for every proposal with its live priced impact, `GET /api/proposals/:id` for one proposal in full, `GET /api/marketplace/:idOrSlug/context` when he genuinely wants the whole brief. He is told those three paths by name, because an assistant that has to search for where the numbers live will decide it already knows.

**`GET /api/marketplace/:idOrSlug/contracts` is the answer to "which proposal is worth approving", and it fits in one read.** A payload that has to be truncated is not an answer, it is a prompt to make five more calls: the floor's own public payload carries 86KB of ballot for a floor with nineteen proposals, an assistant's tool result is capped well below that, and Otto duly opened every proposal one at a time to see the part that was cut off. So this read carries what pricing a decision needs and nothing else: per proposal its id, title, ask, who posted it and whether its decision is still open; per horizon the metric, the date, when it resolves, the two branch prices and their difference, the baseline, and the trades behind each branch. The gist of the pitch, cut to 300 characters, because a reader who cannot tell from the title what the work IS goes and opens the proposal, which is the round trip this read exists to remove. No conversation, no market ids, no pools or volumes. **Live horizons only**, because a settled one cannot be influenced by a decision that has not been made; `?horizons=all` adds them back for anyone reading the record. Open proposals come first and the biggest mover first, and the whole of a real floor lands inside an assistant's tool result rather than being cut in half. **Nothing here is ever cut silently**: the response says how many proposals the floor has, says so when there are older ones outside the brief's window, and says so when a floor grew large enough that the pitches had to go for the prices to fit. The prices never go, because they are the answer.

The cost of this is real and it is the point: a proposal question now spends a round trip it used to skip. What it buys is that the number he quotes was read from the market a second ago rather than from a page assembled before the question was asked, and that the paths he reads apply the same live-pair rule the floor does. `GET /api/proposals/:id` applies it too, so no door into a proposal's pricing hands a reader a retired horizon (`lib/market-pairs.ts`).
...
**Telarchy publishes its public actions log at `/data-room`.** Every public action on the platform (trades, proposals and decisions, deliveries, comments, announcements, readings, books, liquidity, joins, record links, floors opening), newest first, assembled at read time from the tables the product runs on and served as one anonymous, filterable read (`GET /api/data-room/actions`). The page and the endpoint take the same filters, so a person and an agent read the same list. Private floors contribute nothing; emails, addresses, referers and payout details are never on it. Governing doc: `docs/data-room.md`.
...
Approving a proposal means sending real money to a stranger, and the owner
...
`approvedUsd` with the approved proposals that make it up, so "who do I owe and
where do I send it" is one answer instead of two lookups that can disagree.
Declined proposals are on the record and are not money, so they do not count.
...
Email is an interruption a person tunes; it is a bad record. A participant who switched the new-proposal alert off still needs somewhere to see that a proposal went up, and a participant who never opens their mail still needs to find out that their proposal was declined and why. So the floor's top bar carries a bell. It derives the same kinds the mail knows (comment, reply, proposal, settled, decision, and the anyComment firehose), including **a decision on your own proposal**, the answer to the thing the poster is actually waiting for, which the mail also always carries.
...
The inbox is workspace-agnostic: a participant trades on several floors and has one inbox. Each row **lands on the thing it names**, not near it: `/<slug>#proposal=<id>&comment=<id>` opens the floor on that proposal, opens its thread, scrolls to that line and flashes it once; a row with no comment (a new proposal, a decision) flashes the proposal's own headline. "Someone commented on your proposal" that drops the reader on a page where they still have to find the line is barely a link. The flash is an ARRIVAL, never a selected state: a highlight that stays becomes one more thing to dismiss, and the reader already knows what they clicked.

### The charter

Nothing caps what a participant may buy in one market. A trader who is confident deploys their whole balance, and the desk shows the same trade the API accepts: a screen that offers a size the server then refuses is a worse fault than a book one account can push, because it teaches the trader the platform is broken rather than that the price is contested. What bounds a push is the money it costs and the traders on the other side of it; what catches coordination is the trade record, which is public per participant, and the owner's charter, which says when a market is voided.
...
- **Invariant**: a market may only exist while its metric's **definition** (name, description, formula, `marketRangeMax`) is unchanged from when the market was created. The set of valid statuses is:
  ...
  - **voided**: market was cancelled and every participant refunded **what they still had at stake in it**: the sum of their trades on that market (buys positive, sells negative), floored at zero. This is the only correct outcome whenever the metric's definition would change or disappear out from under a market.
    ...
    **A void refunds net cash, not gross cost.** The refund is not `positions.totalCost`, the cumulative BUY cost, which a sell never reduces (selling decrements `shares` only, on purpose); refunding gross cost would let a participant who bought and sold the same shares back mint credits on every round trip before an expected void. Refunding net cash means a break-even round trip gets nothing back, someone still holding gets exactly what they still have in, and the floor at zero means a void never DEBITS anyone. A participant who sold out above their cost keeps that realised gain and receives no refund; the shortfall comes out of pool leftover before LPs, which is where market-maker risk belongs.
...
- `/<slug>` is a market's trading floor, and the root redirects to the
  flagship one. It carries the market, the proposals board, both rails, the
...
- `/login`, `/signup` and `/waitlist` are the doors (`/manage` redirects
  to the home page); `/terms`,
