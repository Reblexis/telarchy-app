...
**noted 2026-08-24 (Viktor)**: leaning toward open-sourcing sooner than the "network is a moat" gate above. His reasoning, verbatim: "if i opensource thats more adoption.. what are the actual cons there? i mean software development will be practically zero cost in the future anyways.. so the only cons imo, is exposing security vulnerabilities and more work duringdevelopment but at the same time.. we want to do taht because thats the best way they can get fixed.." Assessment recorded alongside: the code was never the moat, so the remaining cons are (1) known holes going public before they are fixed (a repo this size attracts scanners, not auditors, so fix first: rotate the master key, scrub git history for secrets, remove the `Function()` eval in formulas, see `docs/infra/launch-security-review.md`), (2) the adoption gain is trust from agent devs and founders, not a self-host wave (a self-hosted instance with no participants is a dead market), (3) MIT is permanent for whatever is published. Not yet decided; no LICENSE committed.
