# Persona: Chen, the admin approving a proposal via conditional markets

Returning admin of an existing workspace. A participant (automated or human teammate) has proposed a proposal, and conditional markets have been trading on it. Chen is about to decide: approve, decline, or negotiate via message thread.

## Context

- **Device**: desktop, 1440x900, Chrome. This is not a first-visit persona; assume logged in and familiar with the sidebar.
- **Referral**: email/Slack/in-app notification about a pending proposal, or simply opens the app and sees proposals in the sidebar.
...
Admin / owner of a workspace with a few metrics (revenue, retention, velocity) and several active markets. A participant or teammate proposed a proposal such as "Hire freelance designer for H2 landing page". Conditional markets spawned for each leaf metric. At least 2-3 participants have traded on the conditional markets.

## Mental model

**They already know**:
- The product enough to navigate: sidebar, workspace context, proposals page, markets page.
- Which metrics in their workspace matter most right now.
- That "approving" a proposal means committing to the action; conditional markets stay open for post-decision tracking.
...
- What happens to positions / staked credits if they decline the proposal.

## Success path

Opens Proposals. Sees the pending proposal with an obvious "conditional forecast" summary. Per-metric baseline vs conditional deltas are displayed in one table. They note which metrics are predicted to improve / worsen. They weigh this against their own priors, then click Approve, Decline, or the message thread to ask a question. Whatever they click, the outcome is clear within 5 seconds.

## Session script

- **T+00:00 — Open Proposals.** Is the pending proposal surfaced at the top? Is there a count badge on the sidebar item? Is there a clear "needs your attention" signal vs fully autonomous ones?
- **T+00:30 — Click the proposal.** Does the detail page show the proposal title, description, proposing participant, and baseline-vs-conditional per-metric forecasts side by side, or does Chen have to click into each market individually?
...
- **T+03:30 — Ask a question via message thread.** Is the proposal thread discoverable on the same page? Is it obvious how to ping the proposer? Does the proposer get notified?
- **T+04:30 — Decision UI.** Approve and Decline buttons. Is the consequence of each spelled out? Specifically: on Decline, are conditional markets voided and stakes refunded? On Approve, does the proposal move to "done" or "in progress"?
- **T+06:00 — Approve.** Click. What happens? Toast? Redirect? Does Chen see the proposal status update? Are conditional markets left running or do they resolve/close?
...
- **Blocker**: proposals tab shows only a raw list with no forecast info; Chen has to click each one to see anything.
...
- **Low**: "proposals" is the only word for decisions. A founder-flavored user might expect "proposals" or "initiatives".

## Conversion criteria

Chen successfully approves or declines the proposal with confidence in what they just did, and the consequence (status change, refund on decline) is visible and consistent with expectations.
...
- This persona requires a seeded state: a workspace with metrics and at least one pending proposal with conditional markets that have real (non-zero) trade volume. If state is not available, precondition the session by using the primary admin account to propose a test proposal, or use an existing dogfood-workspace proposal.
- Verify the *live* flow end to end: approve → conditional markets stay open (per vision) → proposal status changes.
