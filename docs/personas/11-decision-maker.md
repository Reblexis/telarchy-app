...
- **T+01:00 — Marketplace (anonymous).** Does `/marketplace` show any *decisions* in progress (conditional markets tied to tasks), or only flat metric forecasts? A visitor looking for a decision-support tool should see decisions, not just metric tickers.
...
Ends the session having proposed at least one of her two options as a task, and having seen *some* kind of forecasted output (even placeholder or empty-state) tied to her decision. Says "I'd come back with my actual numbers."

## Bounce criteria

Closes the tab without signing up after reading the landing and seeing nothing that matches her concrete decision-making need. Or: signs up and cannot find a "decision" concept anywhere in the signed-in UX within 5 minutes. Or: proposes a task, gets a blank market with no consensus, reads it as "broken", leaves.
