...
Bounce with confusion. The naive "share the workspace URL" path (`telarchy.com/marketplace/<workspaceId>`) renders a blank page. Dev has to pivot to `/signup` → `/create-workspace` funnel → `/marketplace` → find workspace by name → click Join. That works, but Dev has no idea what permissions they got, sees `Edit` / `Delete` buttons on metrics they cannot actually use, sees the sidebar tab "Proposals" as empty even though a proposal exists in the workspace, and has several console 403s dropped on the devtools for endpoints their role can't access.
...
2. T+00:30 — Before logout, the blank page actually shows **`"Inspecting: Hire senior designer full-time ($120k/yr) — showing impact predictions / Back to Proposals / Exit Inspect"`** leaked from the previous logged-in user's localStorage (`inspectProposal` key survives logout). So the blank page can flash with another user's proposal title depending on storage state.
...
8. T+03:30 — Navigate to `/proposals`. Page shows "No proposals yet." `GET /api/proposals` (with workspace header) returns `[]`. But the workspace *does* have a proposal ("Hire senior designer full-time", status=approved). Either the endpoint filters to current-user-only, or to pending-only. For an invitee trying to "see what my boss set up", this is a dead page that hides the thing they came for.
...
10. T+04:30 — Sidebar: the "Settings" link is correctly hidden (admin only). `Check-in`, `Metrics`, `Markets`, `Proposals`, `Agents`, `Sources` are all visible. No badge or copy identifies Dev as a "Trader" vs "Admin". No "you have read/trade permissions" banner.
11. T+05:00 — Dev closes the tab. Mental model: "I think I joined something? It rendered but I can't tell what I'm allowed to touch, and the proposals tab is empty even though my boss told me they set one up."
...
- [blocker] F2 — **`inspectProposal` in localStorage persists across logout.** Anonymous or freshly-logged-in users see a banner "Inspecting: <prior proposal title> - showing impact predictions" leaked from the previous session's state. Cross-user proposal-title leak on a shared machine. Fix: clear app-local storage on logout; or key the inspect state by `authUserId`.
...
- [blocker] F4 — **Proposals tab shows "No proposals yet." for a trader when an approved proposal exists in the workspace.** Either `/api/proposals` filters by proposer, by user-can-approve, or by pending status; whichever it is, a trader reading "what's going on here" sees a blank page. Fix: surface all proposals visible to a participant, with a "pending / approved / declined" filter; optionally show a "you cannot approve these" badge for traders.
- [blocker] F5 — **No role signalled anywhere in UI.** Dev has no visible indicator of being Trader. Persona 12's F3 high becomes blocker in practice because every screen (metrics with Edit/Delete, check-in with Save, proposals with Propose) looks *the same* to a trader as to an admin.
...
- [medium] F13 — **Invitee sees `Propose Proposal` form on `/proposals`.** A trader can probably propose a proposal (at a credit cost), but the form lacks cost/consequence copy. If traders *cannot* propose, the form shouldn't render.
...
Low probability. Persona 12's trust-neutral baseline means the first five minutes decide. Today: blank page on direct link, signup funnels into wrong flow, unclear role, empty proposals view. Dev closes the tab and tells their boss "seemed broken, maybe try again later".
...
2. **Clear `inspectProposal` (and any other user-scoped) localStorage on logout.**
...
4. **Client-side `memberRole` gating** across metrics (Edit/Delete), check-in (Save), proposals (Propose), updates (panel). Hide admin-only affordances.
5. **Show role + permissions banner once on first render of a joined workspace**: *"You're a Trader in Persona11 Test Co. You can forecast on any market but can't change metrics or approve proposals."*
6. **Loosen the Proposals view**: show approved/declined proposals with a filter, not an empty page. This is what "see what the team decided" looks like.
