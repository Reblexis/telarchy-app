...
- **"How this decides" is where the mechanism lives**, and the only place
  ...
  commits (on a proposal with options, "Choosing an option commits
  <proposer> to it.", or "Choosing an option pays <proposer> $N and commits
  them to it." with an ask: the one committed is the proposer, named as the
  proposal names them, never the floor's name); that every number this floor prices gets two markets for this
...
missing sorts last, and impact breaks a tie. The decided proposals are
hidden by default whenever there are any: a board with nothing pending
shows the fold row alone, "7 decided" and SHOW, and nothing above it,
because an empty ballot is still a ballot and the archive never stands in
for it. A board with nothing decided has no fold. Two rules protect the
...
releases that selection, so the control can never be dead. The payload
carries every pending proposal however many were decided after it, and the
newest 40 decided ones for the fold (docs/infra/deploy.md, "A list never
hides a live proposal").
...
besides the ticket is a hairline row, "Add your own trading bot",
opening the Agents page with this market preset on its new-bot form,
`/agents?market=<slug>#agent-setup`: bots are a thing anyone can add, and
the floor says so where the trading happens, not in a footer. It stands
under the ticket in the rail, so it is on screen from the first paint on a
wide screen and under the ticket on a phone, and it is there whether or not
a market is open, since a bot is added to the floor, not to one book.
It is a row between two hairlines, the bot mark, the words in the secondary
...
address on its page; the DECIDING NOW block; and the facts row.

The deciding now block shows the floor's most traded open proposal (pending,
trading not closed; the credits traded on all its books; ties to the newest),
and nothing when it has none. It carries a mono small-caps label in the
accent, "DECIDING NOW", with the countdown to the decision ticking beside it
("decides in 0:24", "3h 12m", "2d 6h"); the proposal's number and title on
