...
| `order` | a participant placed a limit order (buy or sell, side, level, budget or shares), and later it filled, was cancelled, expired or was voided | `limit_orders`: one row at `createdAt`, and one at `updatedAt` for a status other than `open` |
| `liquidity` | a participant put liquidity behind a book, or funded a proposal's books | `liquidity_events` of type `injection`, one row each; of type `proposal-subsidy`, one row per proposal, funder and minute with the amounts summed, because a subsidy lands on every branch book at once. The grouping reads only the newest minutes a page can show (whole minutes, one more than the page's rows) and a page boundary falls between groups, never inside one, so a read costs the size of a page, not of the floor's history. The engine's own `initial` and `anchor` rows are not actions |
