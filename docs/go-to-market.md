...
**Agent-first (primary, adopted 2026-07-12, Viktor).** Telarchy onboarding is something the visitor's own agent performs, not a form the visitor fills in. The landing hero (agentphone.ai-style, revised 2026-07-13, Viktor) pairs a primary "Get started" button (browser signup into the cinematic canvas) with a compact "Copy agent prompt" pill; the prompt itself is hidden behind the pill rather than shown as a card, so it no longer dominates the hero. The prompt is deliberately one sentence: all onboarding behavior lives in the server-side guide, so improving onboarding means editing the guide, never the prompt string. The prompt sends the agent to `GET /api/guides/onboarding`, the canonical server-side walkthrough. It is written as a guided, wizard-style conversation (warm open, small question rounds, progress announcements, decisions reflected back) and it opens with intent: "what do you want to get out of Telarchy?". From there it branches: the **govern path** (personal goals, solo founder, startup, bigger company, a workspace governing an AI agent) creates the account and workspace with the right template and visibility, co-designs metrics and time preferences per the metric-design guide, wires automatic value syncing with correct settlement timing, registers participants, and hands off with a written summary; the **participant path** builds the user a trading or forecasting agent instead (edge question, marketplace registration, the read-trade loop from the recipes guide, P&L and leaderboard observability). Users who want both run govern first, then participant. The guide lives in the backend (`functions/src/routes/guides.ts`) so it updates with the API, and it ends by recommending the Telarchy skill (`Reblexis/telarchy-skill`) for ongoing use.

**Key-first (adopted 2026-07-12, Viktor): onboarding never blocks on account ceremony.** `POST /api/onboard` gives the agent an identity, a workspace, a scoped key, and a one-time claim link in a single unauthenticated call; the human attaches email/OAuth later via `/claim` only if they want the web dashboard (consent is recorded there, and the reduced unclaimed credit grant tops up to the full one). The email/browser flow remains for users who already have accounts.
...
**Confirmed 2026-07-12 (Viktor):** approval stays the wedge, alignment stays the mission, and the wedge never stands alone: "approval layer" spoken bare reads as permission gating (HITL approval queues, guardrails), so the calibrated-number clause is mandatory whenever the wedge is used. Revisit only if real conversations keep triggering the permission-gating read despite the full sentence.
