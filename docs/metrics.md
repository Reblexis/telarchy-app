...
Three of the metrics below are priced on the public floor: profitable forecasters, outside owners deciding, and revenue. Weekly active verified traders and implied valuation are evidence series: recorded, shown, not priced.
...
Until 2026-09-08 the hero metric of the Telarchy floor; since then an evidence series (recorded hourly, shown, not priced), because it counted only participants with a paid Manifold link and the goal is accurate forecasts, not forecasters from one site. `GET /api/marketplace/stats` returns it and the hourly `POST /api/cron/self-sync` records it verbatim on the floor's metric. Distinct participants who (a) have a Manifold account synced (the verified set: each maps to a public Manifold profile anyone can check) and (b) placed trades totalling at least 100 credits in absolute cost across the trailing 7 days, sells included.
