...
Three of the metrics below are priced on the public floor: weekly active verified traders, outside owners deciding, and revenue. Implied valuation and profitable forecasters are evidence series: recorded and shown, not priced. Implied valuation opens no books until an investment closes; profitable forecasters opens no new books, its horizons frozen to the three dates already open so every held position settles on its own date (the retirement recipe: freeze, never clear, because clearing a metric's horizons voids every proposal pair on it).
...
- **Markets:** none new. The horizons are the three absolute dates its books already stand on (`2026-09-10`, `2026-W37`, `2026-09`), so the open books settle on their dates and nothing opens as the clock rolls. Retired from the floor because a count of profitable traders is a supply number, not an accuracy number: an accurate market pays nobody, and a buy in a thin book marks its own spread as profit (record: the telarchy umbrella's `notes/active-profitable-forecasters-2026-09-10.md`, which also proposes the accuracy metrics that replace it). Range 0 to 100.

### Weekly active verified traders (`weeklyActiveVerifiedTraders`)

A priced metric of the Telarchy floor: `GET /api/marketplace/stats` returns it and the hourly `POST /api/cron/self-sync` records it verbatim on the floor's metric. It counts only participants with a paid Manifold link, which is why it is no longer the only engagement number priced there (profitable forecasters, above, counts whoever is right, link or none); the owner keeps both, because who shows up and who is right are different questions and a floor that prices only the second cannot see a week when nobody came (owner decision 2026-09-08, "i still want to keep the active traders metric"). Distinct participants who (a) have a Manifold account synced (the verified set: each maps to a public Manifold profile anyone can check) and (b) placed trades totalling at least 100 credits in absolute cost across the trailing 7 days, sells included.
