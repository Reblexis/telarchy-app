...
goal-horizon: short
goal-statement: |
  As a workspace, when a participant proposes a task, the platform spawns
  conditional markets per relevant metric; on approve those markets resolve
  against the post-task metric, on decline they void and refund stakes.
---

# Browse test: Conditional markets (linked to tasks)

## What this tests

The lifecycle: create task → conditional markets exist → trade on them →
approve or decline task → markets resolve / void cleanly. Maps to
`mvp-evaluation-plan.md` 4.9, the conditional path of `tasks-flow.md`.
...
### T1. Proposing a task creates conditional markets
...
cms=$(curl -sf -H "X-Agent-Key: $KP" -H "X-Workspace-Id: $WS" \
  "$TT_BASE_URL/api/tasks/$TASK" | jq -r '.conditionalMarketIds // [] | length')
[ "$cms" -ge 1 ] || echo "WARN: no conditional markets auto-spawned for task $TASK"
```
...
### T3. Approve task → conditional markets resolve
...
status=$(curl -sf -H "X-Agent-Key: $KP" -H "X-Workspace-Id: $WS" \
  "$TT_BASE_URL/api/tasks/$TASK" | jq -r '.status')
[ "$status" = "approved" ]
...
### T4. Decline path: another task voids its conditional markets
...
mkts2=$(curl -sf -H "X-Agent-Key: $KP" -H "X-Workspace-Id: $WS" \
  "$TT_BASE_URL/api/tasks/$TASK2" | jq -r '.conditionalMarketIds[]?')
...
curl -sf -H "X-Agent-Key: $KA" -H "X-Workspace-Id: $WS" \
  -H 'Content-Type: application/json' -X POST -d '{}' \
  "$TT_BASE_URL/api/tasks/$TASK2/decline" >/dev/null
...
### T5. Conditional market created after task closes is rejected
...
- No coverage of partial-resolve where a task affects only some of N
