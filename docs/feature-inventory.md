...
168. Workspace brief (`GET /api/marketplace/:idOrSlug/context`, `?format=md`): one read with the company, its metrics and history, its markets and prices, its contracts with priced impact, announcements, and owner-published documents. What an outside agent reads instead of scraping the page.
171. Data room (`telarchy.com/data-room`, `GET /api/data-room`): Telarchy's own books, prose and figures in one anonymous read. Vision, the market it runs on itself, traction, traffic (a `traffic_daily` rollup that outlives the 30-day visit purge), the change log generated from git at deploy time, plans and risks. See `docs/data-room.md`.
170. Question log (`floor_questions`, `GET /api/admin/questions`, the Questions section on `/admin`): every question asked of a floor with its answer, its cost, and who asked when they were identified. A row with an error is one nobody could answer.
169. Otto, the floor's market maker (`POST /api/marketplace/:idOrSlug/ask`, the corner chat): a named character who answers from that brief only, holds opinions, gives advice, and never invents a number. Anonymous, multi-turn, per-IP rate limited. Pointing your own agent at the same facts is a copyable prompt in account settings.
