...
169. Ask the floor (`POST /api/marketplace/:idOrSlug/ask`): a plain-language question answered from that brief only, anonymous, per-IP rate limited, with a copyable prompt for pointing your own agent at the same facts.
