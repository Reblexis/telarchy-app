...
| Real browser automation | Playwright MCP | Click, fill, screenshot, keyboard, viewport resize, console/network intercept, navigation — i.e. everything a user does with a mouse and keyboard |
...
1. Completing OAuth consent on Google/GitHub's own domains (Playwright can click "Continue with Google", but the OAuth provider's own consent screen needs an authenticated browser session Claude doesn't have).
...
| # | Check | How | Pass |
| --- | --- | --- | --- |
| 1.1 | Landing page loads under 2 s (cold), under 500 ms (warm) | Playwright `browser_navigate` + network timing | First contentful paint < 2 s on 4G throttle |
| 1.2 | Hero copy is legible and answers "what is this?" | Playwright screenshot | Human-readable without explanation from me |
| 1.3 | Every nav link resolves to a 200 page (no dead links) | Bash: crawl internal links | No 404s |
| 1.4 | Footer has Terms + Privacy links that open the right pages | Playwright click → URL assertion | `/terms` renders ToS, `/privacy` renders PP |
| 1.5 | "Sign up" CTA leads to `/signup` and loads instantly | Playwright click | `/signup` renders without console errors |
| 1.6 | Marketplace counter widget shows real numbers | Playwright + `curl /api/marketplace/stats` | Counter matches API |
| 1.7 | Mobile viewport (375 px) renders without horizontal scroll | Playwright `browser_resize` + screenshot | No overflow, hero still above fold |
| 1.8 | Tablet viewport (768 px) renders cleanly | Playwright `browser_resize` + screenshot | Layout holds |
| 1.9 | Marketplace page loads and lists public workspaces | Playwright navigate `/marketplace` | ≥1 workspace visible, join button works |
...
| # | Check | How | Pass |
| --- | --- | --- | --- |
| 2.1 | Signup form requires email, display name, password, consent checkbox | Playwright: submit with each field empty | Each missing field blocks submit with clear error |
| 2.2 | Consent checkbox is unchecked by default | Playwright state read | `checked=false` on load |
| 2.3 | Terms and Privacy links inside consent label open the right pages | Playwright click | `/terms` and `/privacy` each render |
| 2.4 | Password < 8 chars rejected with specific message | Playwright | Error: "Password must be at least 8 characters" |
| 2.5 | Happy-path signup creates account and lands on `/create-workspace` | Playwright full flow with fresh email | Redirect occurs, no error surface |
| 2.6 | Duplicate email gives clean "account exists" error | Playwright resubmit same email | User-friendly message, no stack trace |
| 2.7 | Backend records `consentedAt` and `consentedVersion` in `user` table | Bash: query DB after signup | Both columns populated |
| 2.8 | OAuth button without consent checked surfaces error, does not initiate OAuth | Playwright click Google without checkbox | Error shown, URL unchanged |
| 2.9 | OAuth with consent checked initiates redirect to provider | Playwright click Google with checkbox | URL becomes `accounts.google.com/...` |
...
| 2.13 | Welcome CheckIn page shows seed-liquidity receipt + "bots arrive ~5 min" banner | Playwright screenshot `/check-in?welcome=1` | Both elements visible |
| 2.14 | Email leak check: display name is not filled with email anywhere | Playwright: inspect user-visible text in dashboard/sidebar | No `@` character renders as a name |
| 2.15 | Session cookie survives reload | Playwright reload → `/api/auth/me` | Still authenticated |
| 2.16 | Logout clears cookie and redirects | Playwright sidebar logout | Cookie gone, on public page |
...
| # | Check | How | Pass |
| --- | --- | --- | --- |
| 3.1 | `/create-workspace` renders template options | Playwright screenshot | At minimum `startup`, `personal`, `blank` visible |
| 3.2 | Each template creates a workspace with a sensible default metric set | Playwright create → inspect metrics list | Metrics exist, have non-zero values, formulas resolve |
| 3.3 | Workspace visibility picker defaults to Open (per recent commit) | Playwright inspect defaults | Open selected |
| 3.4 | Create custom metric (name, value, formula) via UI | Playwright fill form → submit | Metric appears in list within 1 s |
...
| 3.7 | "Markets →" drill-down on leaf metric filters markets page | Playwright click | `/markets?q=<name>` and filter input pre-filled |
...
| 4.10 | UI trade panel updates consensus live after trade | Playwright | Number updates within 2 s |
| 4.11 | Position panel shows owned shares correctly | Playwright | Shares match API |
...
| 5.5 | Agent portal UI hides USDC settings when flag is off | Playwright as agent key | Only "disabled" notice shown in Settings |
...
| 6.2 | Join flow from `/marketplace` UI works for authed user | Playwright | Shows up in `My workspaces` |
...
| 6.4 | Workspace visibility picker (Private/Public/Open) updates group capabilities | Playwright toggle → Bash verify group | `Public` group has expected capability set |
...
| 7.13 | XSS spot-check: metric name with `<script>` renders as text | Playwright create + view | Literal string shown, no execution |
...
| 8.4 | Frontend AccountPage hides deposit UI when flag off | Playwright | No deposit section rendered |
| 8.5 | Frontend AgentPortal hides deposit UI when flag off | Playwright | "disabled" notice shown instead |
...
| 9.3 | `/terms` and `/privacy` render the markdown cleanly | Playwright | Headings, lists render; no raw `#` visible |
...
| # | Check | How | Pass |
| --- | --- | --- | --- |
| 12.1 | Every form input has an associated `<label>` | Playwright + DOM query | No bare `<input>`s |
| 12.2 | Tab order through signup is logical | Playwright `browser_press_key Tab` sequence | Focus progresses email → name → password → checkbox → submit |
| 12.3 | Buttons have discernible text (not icon-only without aria-label) | DOM query | All have text or aria-label |
| 12.4 | Color contrast for body + link text | Playwright screenshot + heuristic | Passes WCAG AA visually (no near-invisible text) |
| 12.5 | No console errors or warnings on main flows | Playwright `browser_console_messages` during signup + dashboard | Empty |
| 12.6 | No network 404s on main flows | Playwright `browser_network_requests` | None for app's own routes |
| 12.7 | **Screen-reader run-through of signup + one trade** `(human)` | VoiceOver/NVDA | All controls announced with intent |
| 12.8 | Focus visible on every interactive element | Playwright tab + screenshot | Outline present |
...
| # | Check | How | Pass |
| --- | --- | --- | --- |
| 13.1 | Signup form usable at 375 × 667 | Playwright resize + screenshot | All fields tappable, no horizontal scroll |
| 13.2 | Dashboard readable at 375 px | Playwright | Cards stack, numbers legible |
| 13.3 | Marketplace readable at 375 px | Playwright | List renders |
| 13.4 | Settings/account pages usable at 375 px | Playwright | All controls reachable |
| 13.5 | iPad-portrait layout (768 px) not degenerate | Playwright | Looks intentional, not "narrow desktop" |
...
| # | Check | How | Pass |
| --- | --- | --- | --- |
| 14.1 | Create 10k metrics via API, dashboard still loads | Bash + Playwright | Render time < 5 s, no timeout |
...
| 14.4 | Script-injection in task title/description | Bash create + Playwright render | Escaped |
...
| 15.2 | Guides pages (`/guides/*`) all render | Playwright navigate each section | Markdown renders |
| 15.3 | Sources (text + GitHub) end-to-end if enabled | Bash + Playwright | Text source creatable + readable; GitHub flow reachable |
| 15.4 | Tasks propose → approve → payout works end-to-end | Bash/Playwright | Balance shifts by `price` credits |
| 15.5 | Credit balance displayed consistently across Dashboard, Marketplace, AgentPortal | Playwright compare | Three values identical |
