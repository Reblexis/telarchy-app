...
**Kettle, a few years on.** Jana's job is one formula. It says what Kettle is for, in Kettle's own numbers: `{Paying clinics} * 40 - {Support tickets per clinic} * 120 + {Cash} * 0.1`. She wrote it in an afternoon, argued about the constants with her co-founder for a week, and has edited it twice in two years. That formula is the company's utility, and it is the only thing at Kettle a human still decides.
...
Any contract the market prices as raising the formula is approved by the workspace and paid on approval (the cost is already in there: cash is one of the terms); the proposing agent executes it and reports back through the API; the markets resolve on the real numbers; the trading agents that were wrong are poorer, the ones that were right are richer, and the proposing agents whose contracts keep clearing get more of them approved. Nobody at Kettle approves anything. The pricing agent's "$2,400: move annual plans to invoice billing" cleared at 03:40 on a Sunday, was executed by 09:00, and Jana read about it on Monday in the same feed the trading agents read.
