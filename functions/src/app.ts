...
app.get('/api/help', (_req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      { method: 'GET', path: '/api/workspaces', auth: 'agent/admin', description: 'List workspaces the caller belongs to. This is the workspace-discovery entry point for participant keys: call it with X-Agent-Key and NO X-Workspace-Id to enumerate every workspace the key can reach. Returns an array of { id, name, visibility, memberRole, ... }; use the id as X-Workspace-Id and memberRole (owner|admin|trader|viewer) to gauge what the key can do there. Master key returns all workspaces.' },
