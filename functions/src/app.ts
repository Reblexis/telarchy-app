...
app.get('/api/help', (_req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      { method: 'GET', path: '/api/agents/:idOrNickname/public', auth: 'optional', description: 'Public participant profile. Auth is optional; pass a session cookie, X-Agent-Key, or X-API-Key + X-Workspace-Id to widen the detail visible. :idOrNickname is matched against agents.id first, then case-insensitively against agents.nickname. Returns { id, nickname, intent, joinedAt, stats: { rank, calibration, accuracy, totalEarnings, resolvedMarkets, totalTrades, lastTradeAt }, activeWorkspaces: [{id, name}], openPositions: [{ workspaceId, workspaceName, marketId, metricName, targetDate, direction, shares, totalCost, status, probabilityHigher, consensus, actualValue }], recentTrades: [{ id, workspaceId, workspaceName, marketId, metricName, targetDate, direction, kind, shares, cost, createdAt }] }. Stats and activeWorkspaces are aggregated only over public-visibility workspaces (privacy contract shared with /api/leaderboard). openPositions and recentTrades expand to include any workspace where the caller has the read capability; master key and platform admin see everything. Recent trades are capped at 20, newest first.' },
