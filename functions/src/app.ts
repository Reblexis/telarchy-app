...
app.get('/api/help', (_req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      { method: 'GET', path: '/api/agents/:idOrNickname/public', auth: false, description: 'Public participant profile. No auth. :idOrNickname is matched against agents.id first, then case-insensitively against agents.nickname. Returns { id, nickname, intent, joinedAt, stats: { rank, calibration, accuracy, totalEarnings, resolvedMarkets, totalTrades, lastTradeAt }, activeWorkspaces: [{id, name}] }. Stats are aggregated only over public-visibility workspaces, matching the privacy contract of /api/leaderboard. Participants with no resolved positions get null calibration and accuracy.' },
