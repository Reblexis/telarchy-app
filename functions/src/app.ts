...
app.get('/api/help', (_req, res) => {
  res.json({
    ...
    concepts: {
      ...
      conditional_market: 'A market that opens conditional on a proposed task being approved. The killer use case: any participant calls POST /api/tasks with a proposed action and a price, and conditional clones of every active leaf-metric market spawn under that task (tagged with taskId). Participants forecast on the conditional markets to signal "what will metric X look like if this task is completed?" The owner sees per-metric impact (conditional consensus vs baseline consensus) and approves or declines on the forecast, not the pitch. Approve: proposing participant earns the price; conditional markets remain live. Decline: conditional markets are voided, all stakes refunded.',
