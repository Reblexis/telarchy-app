...
app.get('/api/help', (_req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      { method: 'GET', path: '/api/marketplace/workspaces/public', auth: false, description: 'List of public workspaces. Returns [{ workspaceId, name, visibility, proposalReward, spamPenalty, maxPendingProposalsPerParticipant, metricCount, openMarketCount, proposalStats: { total, approved, declined, declinedSpam, withdrawn, pending } }]. metricCount (metrics defined) and openMarketCount (markets still tradeable) let an agent tell empty workspaces from active ones before joining. proposalStats counts proposals created in the last 30 days; proposers can use it to gauge owner review behaviour before submitting.' },
