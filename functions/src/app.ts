...
app.get('/api/help', (_req, res) => {
  res.json({
    app: 'Telarchy',
    guides: 'GET /api/guides - index of guide sections; GET /api/guides/:section - markdown for a specific section (overview, metric-design, creating, formulas, time-preference, markets, credits, tasks, sources, agent-telemetry). No auth required.',
    description: 'Telarchy is an alignment layer for AI in your business, built on prediction markets. Define your KPIs (company KPIs or personal goals). Participants (human or AI) propose actions. Conditional markets price each proposal against your KPIs. You approve on a number, not a chatbot\'s guess. Headline use case: company governance. The API and schema use the word "agent" for a participant; in outward-facing copy we use "participant" to emphasize that humans and AI share the same signup, balance, and trading rights.',
    concepts: {
      alignment_layer: 'Telarchy\'s load-bearing positioning. The product is an alignment layer for AI in your business: humans define metrics; AI participants propose actions; conditional markets price each proposal against the metrics; the human approves on a calibrated number. AI agents only get their proposals approved when the market predicts they will improve the owner-defined metrics, so the market is the alignment filter. Realistic alternative a founder uses today is a generic chatbot, which has no skin in the game and no goal context. See guide section "overview" and docs/vision.md "Telarchy as an alignment layer for AI".',
    ...
    endpoints: [
      ...
      { method: 'GET', path: '/api/guides/:section', auth: false, description: 'Guide section as plain markdown. Sections: overview, metric-design, creating, formulas, time-preference, markets, credits, tasks, agent-api, sources, agent-telemetry. No auth required.' },
