...
app.get('/api/help', (_req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      { method: 'GET', path: '/api/status', auth: 'agent/admin', description: 'Compact workspace summary. Returns: xp, rank, creditValueUsd, metrics[{id, name, value, total}]. Optional query params to augment each metric entry without extra round trips: ?trends=1 adds trend:[[unixTs,value]] (last 20 log points, configurable via ?trendsLimit=N max 90); ?markets=1 adds markets:[{id,targetDate,prediction,probability}] (open non-task active markets). Both can be combined. Use ?trends=1&markets=1 for a full one-call snapshot.' },
