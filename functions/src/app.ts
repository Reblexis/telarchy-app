...
app.get('/api/help', (_req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      { method: 'POST', path: '/api/predictions/markets', auth: 'admin', description: 'Create a one-off manual market. Body: { metricId, targetDate, rangeMin?, rangeMax?, liquidity?, skipAutoLiquidity? }. When workspace auto-fund is on, debits the workspace owner agent unless skipAutoLiquidity is true. Manual markets on metrics without a timePreference config are left alone by the daily refresh (never deactivated, recreated, or rolled); for system-maintained recurring horizons use timePreference.customHorizons on the metric instead.' },
