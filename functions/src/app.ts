...
app.get('/api/help', (_req, res) => {
  res.json({
    app: 'Telarchy',
    guides: 'GET /api/guides - index of guide sections; GET /api/guides/:section - markdown for a specific section (overview, metric-design, creating, formulas, time-preference, markets, credits, proposals, agent-api, auth-and-keys, recipes, api-reference, sources, agent-telemetry, feedback). No auth required.',
    ...
    endpoints: [
      ...
      { method: 'POST', path: '/api/proposals', auth: 'agent/admin', description: 'Propose a proposal. Body: { title, description?, liquiditySubsidy? }. Subject to an optional per-participant pending-proposals cap (workspace.maxPendingProposalsPerParticipant; 0 disables, which is the default); exceeding it returns 429 with { pending, cap }.' },
