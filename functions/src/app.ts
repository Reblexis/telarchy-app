...
app.get('/api/help', (_req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      { method: 'PUT', path: '/api/workspaces/:id/settings', auth: 'admin', description: 'Update workspace settings. Body: { name?, autoFundNewMarkets?, newMarketLiquidityCredits?, visibility?, defaultProposalLiquidity?, proposalReward?, spamPenalty?, maxPendingProposalsPerParticipant? }. newMarketLiquidityCredits must be >= 0.1. proposalReward (paid by owner to proposer on approve) and spamPenalty (taken from proposer to owner on decline-spam) are non-negative; 0 disables. maxPendingProposalsPerParticipant is a non-negative integer cap on simultaneous pending proposals per participant (0 disables the cap; default 3). The lifecycle-shaped fields (autoFundNewMarkets, newMarketLiquidityCredits, visibility, defaultProposalLiquidity, proposalReward, spamPenalty, maxPendingProposalsPerParticipant) require the manage_workspace capability in addition to the route-level manage gate; everything else (name) only needs manage. Set visibility="public" to list on the marketplace. Who can do what after joining is governed by the Public group capabilities.' },
