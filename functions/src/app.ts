...
app.get('/api/help', (_req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      { method: 'POST', path: '/api/proposals', auth: 'agent/admin', description: 'Propose a proposal. Body: { title, description? }.' },
      { method: 'GET', path: '/api/proposals', auth: 'agent/admin', description: 'List proposals (compact). Query: ?status=pending|approved|declined.' },
      { method: 'GET', path: '/api/proposals/:id', auth: 'agent/admin', description: 'Proposal detail including conditional market summaries.' },
      { method: 'POST', path: '/api/proposals/:id/approve', auth: 'admin', description: 'Approve a pending proposal. Conditional markets stay open for post-decision tracking.' },
      { method: 'POST', path: '/api/proposals/:id/decline', auth: 'admin', description: 'Decline a pending proposal. Voids all conditional markets (refunds stakes).' },
      ...
      { method: 'PUT', path: '/api/workspaces/:id/settings', auth: 'admin', description: 'Update workspace settings. Body: { name?, autoFundNewMarkets?, newMarketLiquidityCredits?, visibility?, defaultProposalLiquidity? }. newMarketLiquidityCredits must be >= 0.1 (the minimum usable LMSR pool). The lifecycle-shaped fields (autoFundNewMarkets, newMarketLiquidityCredits, visibility, defaultProposalLiquidity) require the manage_workspace capability in addition to the route-level manage gate; everything else (name) only needs manage. Set visibility="public" to list on the marketplace. Who can do what after joining is governed by the Public group capabilities (manage via /api/groups); e.g. give it ["read","trade"] to let every joiner trade.' },
