 ...
 * returning the last price it ever had. On a contract nobody has decided that
 ...
 * moved to a weekly cadence and every contract kept printing its old monthly
 * number). On a DECIDED contract the same rows are the record of what was
 * priced when the owner ruled, so they stay.
 *
 * The ballot learned this in August 2026 and the brief did not, which is how
 * the floor's own market maker came to recommend a contract on a +11.79 the
...
/** True where a contract's voided pairs are its record rather than its forecast. */
 ...
 * contract. Branch-level, not pair-level, because that is the grain the
