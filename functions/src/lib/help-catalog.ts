...
export const HELP: { endpoints: HelpEndpoint[]; [key: string]: unknown } = {
  ...
  endpoints: [
    ...
    },
    {
      ...
      auth: false,
      description:
        'The present state of the workspace\'s live feed (the liveFeed setting, PUT /api/workspaces/:id/settings), proxied: the body is the upstream\'s <url>/state JSON passed through unchanged, so its shape is the feed kind\'s (for kind "snake": { game: { snake: [{x,y}] head first, food: {x,y}, heading, length, step, deaths, complete, size, gameNumber }, grid, gameNumber, next: { action, direction, decided, seconds }, open: { step, decideAt, deadline, cells, directions, proposals: { forward|left|right: { id, title, url } }, quotes: { forward|left|right: { m1, m5, m60: { approved, declined } } } }, recentDecisions, recentTrades, commentary, complete, nextGameAt }; see the telarchy-snake docs, "The feed"). Held by the app for 2 seconds per workspace with one upstream fetch in flight at a time, a 5-second upstream timeout, cache-control: no-store to the caller. 404 when the workspace has no live feed, 403 when it is private, 502 { error } when the upstream fails or answers non-JSON. Public, no key: the floor polls it every 2 seconds while its LIVE segment is on screen.',
    },
    {
      method: 'GET',
