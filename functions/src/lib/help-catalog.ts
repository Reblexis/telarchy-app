...
export const HELP: { endpoints: HelpEndpoint[]; [key: string]: unknown } = {
  ...
  endpoints: [
    ...
    },
    {
      ...
      auth: 'self/admin',
      description:
        'Trade history for a participant in this workspace. Query: ?limit=N (default 100, max 500). Returns id, marketId, metricName, targetDate, direction, kind ("buy"|"sell"), shares (absolute), cost, marketStatus, createdAt. Use :id = me for the authenticated participant.',
    },
    {
      method: 'GET',
    ...
    },
    {
      ...
      auth: 'agent/admin',
      description:
        'Trade history for a market. Query: ?last=N (most recent N trades only). Returns: direction, shares, cost, consensus (market consensus right after the trade, with liquidity injections replayed so the final point equals the live consensus), createdAt.',
    },
    {
      method: 'GET',
    ...
    },
    {
      ...
      auth: 'agent/admin',
      description: 'Proposal detail including conditional market summaries and declineReason when declined.',
    },
    {
      method: 'POST',
    ...
    },
    {
      ...
      auth: false,
      description:
        'Public read of who holds what and the recent trade history for a market (?marketId=) on an Open public workspace. Returns { consensus, positions: [{ handle, id, direction, shares, cost, worth }] (marked to current price, top 50 by size), trades: [{ id, handle, direction, kind, shares, cost, createdAt }] (newest 50) }.',
    },
    {
      method: 'GET',
