...
export const HELP: { endpoints: HelpEndpoint[]; [key: string]: unknown } = {
  ...
  concepts: {
    ...
    conditional_market:
      'A pair of markets that price a proposed action against the same metric: one priced under the assumption the proposal is approved (`branch="approved"`), one under the assumption it is declined (`branch="declined"`). Every proposal spawns BOTH branches for every active leaf-metric (tagged with proposalId). Each branch PRICES THE DIFFERENCE from the baseline market\'s forecast (`quotes: "difference"`): its range is half the metric\'s range either side of zero, its consensus is that difference (`impact`), it opens at 0 (the approved branch at minus askUsd on a net-money metric), and the baseline moving never moves it. The level a branch reads as is `baselineConsensus + impact`. When the owner decides, the baseline\'s forecast at that instant is recorded on the pair as `reference` and the surviving branch settles at the actual value minus it, clamped to its range. Pairs from before difference pricing that anyone had traded keep pricing the level (`quotes: "level"`) and settle at the actual value. The headline impact is `approved.impact - declined.impact` (`approved.consensus - declined.consensus` on a level pair), isolating the causal effect of approving rather than the natural-trajectory baseline (which can be contaminated by traders already pricing in expected approval). Lifecycle: approve = void declined branch (refund), keep approved branch live to resolve against actual KPI at target date; decline = void approved branch, keep declined branch live to resolve and feed counterfactual calibration; withdraw / spam-decline = void both branches. Trade routing by metric + targetDate + proposalId accepts an optional `branch` param (default "approved" for back-compat).',
    prediction:
  ...
  endpoints: [
    ...
    },
    {
      ...
      auth: 'admin',
      description:
        "Refresh markets. Without body: refresh TP markets (create missing, deactivate stale, void duplicates). With body { proposalId }: recreate conditional markets for that proposal. New conditional pairs price the difference from the baseline and open at 0 (the approved branch at minus the proposal's askUsd on a net-money metric, since approval burns the ask into the resolving metric); an off-centre open sizes the LMSR b down from the subsidy so it stays exactly solvent. An older, untraded level pair of a pending proposal is reopened as a difference book here, free; a traded one is left as it is. Returns { created, deactivated, deduplicated }.",
    },
    {
      method: 'POST',
