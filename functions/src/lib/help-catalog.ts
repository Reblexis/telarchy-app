  ...
  endpoints: [
    ...
    },
    {
      ...
      body: {
        ...
        timePreference:
          '{ enabled: boolean, halfLife: number (years, required when enabled), density?: number, customHorizons?: string[], horizonCredits?: { [entry]: { book?: number | null, proposal?: number } } } (optional; customHorizons entries are rolling offsets "+Nmin" (N 1..1440, the minute cell N minutes after the current one)/"+Nh"/"+Nd"/"+Nw"/"+Nm" (months)/"+Ny" or one-shot absolute dates "YYYY"/"YYYY-MM"/"YYYY-Www"/"YYYY-MM-DD"/"YYYY-MM-DDTHH" (hour, UTC)/"YYYY-MM-DDTHH:MM" (minute, UTC; one minute long, settles on the last reading inside it), max 24; a floor on minute horizons calls POST /api/predictions/markets/refresh { force: true } and POST /api/predictions/resolve once a minute; horizonCredits is keyed by those entries and says, in credits, what the BOOK on that date opens with (book; null or absent falls back to liquidityCredits on the metric, then the workspace default) and what a PROPOSAL branch on that date opens with (proposal; default 0, meaning the proposer funds their own), both paid by the workspace owner as the market opens, every time the date comes round; keys that name no entry are dropped; see GET /api/guides/time-preference)',
      },
    },
    {
      method: 'PUT',
    ...
    },
    {
      ...
      auth: false,
      description:
        "The prices of every open book on a public floor, and nothing else, for a caller that watches prices once a second. :workspaceId accepts the workspace id or its slug. Returns { asOf, version, books: [{ marketId, consensus, probability, pool, tradeCount, orders }] }: books are the open baseline books and every book of a pending proposal (both branches, or one book per option on a proposal with options), a few hundred bytes. consensus is the call on the book's own scale (null while the book has no liquidity), probability the same as a fraction of the range, pool the credits in the pool, tradeCount the rows that moved the book (the same count as a dry run's basis.tradeCount). orders is every open limit order resting on the book, anonymous: [{ id, side, direction, limitValue, left (credits on a buy, shares on a sell), holder (a number standing for one participant within that book and that answer), held: { higher, lower } (that participant's shares on the book), expiresAt (only when set) }], so a caller can run the fill pass itself and see where a trade comes to rest after the orders it crosses fill (docs/limit-orders.md, \"A quote lands where the price comes to rest\"). Placing or cancelling an order moves the version. asOf is the instant the server last knew these prices to be current. The response carries an ETag and version is the same value, a hash of the books: send it back as If-None-Match and a floor whose prices have not moved answers 304 with no body. The server answers from memory while prices stand still and reads the database once when they move, however many callers are polling, so polling once a second is expected and this route sits outside every rate limit. It resolves no credentials: every caller reads it as a stranger. Same disclosure rule as GET /api/marketplace/:workspaceId: 404 for an unknown floor, 403 when the floor is not public or its Public group lacks read. Freshness: a trade taken by any instance of the service reaches this read within about a second.",
    },
    {
      method: 'GET',
