  ...
  endpoints: [
    ...
    { method: 'GET', path: '/api/metrics/:id', auth: 'public-read', description: 'Get a single metric by ID.' },
    {
      ...
      body: {
        ...
        description: 'string',
        value:
          'number, or null for an explicit N/A reading (the number does not exist yet; markets settling on N/A void and refund). Default 0.',
    ...
    },
    {
      ...
      body: {
        ...
        description: 'string',
        value:
          'number, or null for an explicit N/A reading: whoever updates the metric may say the number does not exist right now, the reading is logged as N/A, the metric (and every composite referencing it) reads N/A, and a market whose boundary reading is N/A VOIDS (all positions refunded, reason published) instead of settling',
        formula: 'string',
        oldValue: 'number (previous value, for update history; null when the previous reading was N/A)',
