...
export const HELP: { endpoints: HelpEndpoint[]; [key: string]: unknown } = {
  ...
  endpoints: [
    ...
    },
    {
      ...
      auth: false,
      description:
        'What the owner of Telarchy has committed to and by when: the calendar of ONE floor, the platform\'s own (DATA_ROOM_WORKSPACE_SLUG, default "telarchy"), as the data room draws it between the stamp and the filter bar (docs/data-room.md, "What is planned"). Returns { workspace: { id, slug, name } | null, now, items } where items are exactly what GET /api/marketplace/:idOrSlug/timeline returns for that floor (same function, same order: soonest end first, undated last) and now is the server clock. When no public or unlisted floor carries that slug (a fresh instance, or the floor taken private) it is 200 with workspace null and no items, never an error: the room must always open. Open to every origin, no key. The log says what happened; this says what is supposed to happen next.',
    },
    {
      method: 'GET',
