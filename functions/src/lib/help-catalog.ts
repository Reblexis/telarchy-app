...
export const HELP: { endpoints: HelpEndpoint[]; [key: string]: unknown } = {
  ...
  endpoints: [
    ...
    },
    {
      ...
      auth: 'public-read',
      description:
        'List all metrics with computed totals and depths, sorted by depth then order. Each carries settlementSummary (the floor\'s "Settles on:" line, or null when the owner has not written one).',
    },
    {
      method: 'GET',
      path: '/api/metrics/:id',
