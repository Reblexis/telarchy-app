 ...
 *  the 2026-08-30 rebuild added seasons, limit orders and proposals). */
  ...
  endpoints: [
    ...
    },
    {
      ...
      body: {
        ...
        timePreference:
          '{ enabled: boolean, halfLife: number, density?: number, customHorizons?: string[], horizonCredits?: {...}, horizonTitles?: {...} } | null (optional; sent whole, so omitting horizonCredits or horizonTitles clears them; see POST /api/metrics)',
      },
    },
    {
      method: 'DELETE',
    ...
    },
    {
      ...
      auth: 'agent',
      description:
        'Every edit made to a proposal, oldest first: [{ field ("title"|"description"|"askUsd"), oldValue, newValue, at }]. Append-only; a revision cannot be un-made.',
    },
    {
      method: 'DELETE',
      path: '/api/proposals/:id',
    ...
    },
    {
      ...
      auth: false,
      description:
        "WHICH PROPOSAL IS WORTH APPROVING, in one read that fits. Returns { workspaceId, slug, name, horizons: \"live\"|\"all\", contractsTotal (counted up to 10,000), olderContractsOmitted?, descriptionsOmitted?, contracts: [{ id, title, description (the first 300 characters, with descriptionTruncated when cut), askUsd, status, decisionOpen, proposedBy, impact: [{ metricName, targetDate, resolvesOn, settled?, approved, declined, delta, baseline, approvedTrades, declinedTrades, options? }] }] }. A proposal with options carries options and decidedOption, and each impact row then has approved and declined null, options: [{ id, label, marketId, consensus, trades, delta }] (delta = that option's consensus minus the best other) and delta = the leader's lead. This is the brief's proposal pricing with the conversation and the market plumbing (ids, pools, volumes, probabilities) left out and the pitch cut to its gist, because those are what make the full payloads too large to read in one go: GET /api/marketplace/:idOrSlug carries the same answer inside ~86KB on a floor with nineteen proposals, which an assistant's tool result truncates. LIVE HORIZONS ONLY by default, since a horizon that has already resolved cannot be influenced by a decision nobody has made; ?horizons=all adds them back, marked settled. decisionOpen is true only while an approval would still change something, and a pending proposal's voided pairs are dropped exactly as on the ballot while a decided proposal keeps them. Proposals still open for a decision come first, biggest mover first within each. Use GET /api/proposals/:id when you want one proposal's pitch or conversation, and GET /api/marketplace/:idOrSlug/context when you want the whole brief. Nothing here is ever silently cut: the response carries contractsTotal, sets olderContractsOmitted when the floor has more proposals than the brief's newest-25 window holds, and sets descriptionsOmitted when the floor is large enough that the pitches had to go so the prices would fit. PUBLIC workspaces whose Public group grants read; unlisted and private 403 anyone who is not a member.",
    },
    {
      method: 'GET',
    ...
    },
    {
      ...
      auth: 'platform admin',
      description:
        "Cron entry point: resolve all markets whose period has fully passed, then write daily participant snapshots (one per UTC day, idempotent): the balance and the board's marked profit over public floors. Triggered hourly at minute 0.",
    },
    {
      method: 'POST',
