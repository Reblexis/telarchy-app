 ...
 * opens at the baseline minus the proposal's ask, because approving a $200
 * proposal burns $200 out of a revenue metric the day it is paid. That is
 ...
 * and printed an identical fake negative impact on every proposal in
...
/** Whether a proposal's USD ask is denominated in this metric's own units. */
export function isMonetaryMetric(metricName: string): boolean {
  return metricCurrencyUnit(metricName) !== '';
}

/**
 * Whether approving a proposal mechanically moves this metric down by its
 ...
 * of approved proposals, so approving burns the ask into it the day it is
