 ...
 *
 * The hold is the shared TTL cache (lib/ttl-cache.ts), so it carries its
 * rules: one computation per query at a time even when one takes longer than
 * the hold, the page it replaces answers while it runs, and a failure waits
...
const held = ttlCache({
  ttlMs: ACTIONS_HOLD_MS,
  // Kept from growing with workspaces nobody reads any more.
  maxEntries: 500,
  keyOf: (query: ActionsQuery) => holdKey(query) ?? '',
  load: (query: ActionsQuery) => buildActions(query),
});
...
export function buildActionsHeld(query: ActionsQuery): Promise<ActionsPage> {
  if (holdKey(query) === null) return buildActions(query);
  return held.get(query);
}

/**
 * How far back a read across floors reaches (docs/data-room.md, "Filtering"):
 * a read by participant, or over every floor, names no floor, so no floor's
 * index bounds it and machine-run floors are in it. Thirty days before its
 * newest instant, and each page from its own cursor.
 */
export const CROSS_FLOOR_REACH_MS = 30 * 86_400_000;
...
export async function buildActions(query: ActionsQuery): Promise<ActionsPage> {
  ...
  if (!workspaceId && (actorId || query.floors === 'all')) {
    const newest = Math.min(query.before?.getTime() ?? Infinity, cursor?.at.getTime() ?? Infinity, Date.now());
    const reach = new Date(newest - CROSS_FLOOR_REACH_MS);
    if (!after || after < reach) after = reach;
  }
  ...
  const common = (at: SQL, id: SQL, ws: SQL | null, actor: SQL | null, withCursor = true): SQL => {
    ...
    if (after) parts.push(sql`${at} > ${utc(after)}::timestamp`);
  ...
  // The decision instant is proposals.decided_at, generated by Postgres from
  // the status and the instants the decision paths write (db/schema.ts), so a
  // floor's decisions come newest first off proposals_ws_decided_idx instead
  // of being sorted out of its whole history.
  add(
    'decision',
    sql`SELECT 'decision:' || p.id AS id, p.decided_at AS at, 'decision' AS kind, p.workspace_id,
      ...
      FROM proposals p
      WHERE p.decided_at IS NOT NULL AND p.status IN ('approved', 'declined', 'declined_spam', 'lapsed', 'withdrawn')
        AND ${common(sql`p.decided_at`, sql`'decision:' || p.id`, sql`p.workspace_id`, sql`CASE WHEN p.status = 'withdrawn' THEN p.proposed_by WHEN p.status = 'lapsed' THEN NULL ELSE p.resolved_by END`)}
      ORDER BY p.decided_at DESC, id DESC LIMIT ${take}`,
  );
  ...
  //
  // The cutoff itself is the (take+1)th newest funded minute. Grouping every
  // funding row by minute to find it read the floor's whole history, so it is
  // found by walking back one minute at a time, each step one probe of
  // liquidity_events_ws_created_idx: take+1 probes, whatever the history.
  const subsidyRows = (alias: string) =>
    sql.raw(`${alias}.type = 'proposal-subsidy' AND ${alias}.agent_id IS NOT NULL`);
  const subsidyScope = (alias: string) =>
    sql`${subsidyRows(alias)} AND ${common(sql.raw(`${alias}.created_at`), sql`''`, sql.raw(`${alias}.workspace_id`), sql.raw(`${alias}.agent_id`), false)}${
      cursor ? sql` AND ${sql.raw(alias)}.created_at <= ${utc(cursor.at)}::timestamp` : sql``
    }`;
  const subsidyCutoff = sql`COALESCE((WITH RECURSIVE funded_minutes(m, n) AS (
        SELECT (SELECT date_trunc('minute', max(l2.created_at)) FROM liquidity_events l2 WHERE ${subsidyScope('l2')}), 1
        UNION ALL
        SELECT (SELECT date_trunc('minute', max(l3.created_at)) FROM liquidity_events l3
                WHERE ${subsidyScope('l3')} AND l3.created_at < funded_minutes.m), funded_minutes.n + 1
        FROM funded_minutes WHERE funded_minutes.m IS NOT NULL AND funded_minutes.n <= ${take}
      ) SELECT m FROM funded_minutes WHERE n = ${take + 1}), '-infinity'::timestamp)`;
  add(
    'liquidity',
