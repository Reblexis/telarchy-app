...
// ---------------------------------------------------------------------------
// The metrics page (docs/metrics-page.md)
// ---------------------------------------------------------------------------

export interface MetricsOverviewHorizon {
  marketId: string;
  targetDate: string;
  /** When the market settles, so the page can print the date the words mean. */
  settlesOn: string;
  pool: number;
  trades: number;
}
...
export interface MetricsOverviewRow {
  ...
  rangeMin: number;
  rangeMax: number;
  /** null = the workspace default, stated once at the top of the page. */
  credits: number | null;
  /** True while the decay curve picks the dates, which makes the chips read-only. */
  curve: boolean;
...
/**
 * Everything the owner's metrics page draws, in one call: each metric, the
 * dates it is priced for, and what each of those markets holds. Composing it in
 * the browser would be two calls plus a join over markets the page has no other
 * use for, and the join is where a client would get the "which market belongs to
 * this date" rule subtly wrong.
 */
export async function getMetricsOverview(workspaceId: string): Promise<MetricsOverview> {
  const { workspaces, trades: tradesTable } = await import('../db/schema');
  const [rows, marketRows, ws] = await Promise.all([
    db
      .select()
      .from(metrics)
      .where(eq(metrics.workspaceId, workspaceId))
  ...
  const openMarkets = marketRows.filter(m => !m.proposalId);
  const tradeCounts = new Map<string, number>();
  if (openMarkets.length > 0) {
    const counted = await db
      .select({ marketId: tradesTable.marketId })
      .from(tradesTable)
      .where(eq(tradesTable.workspaceId, workspaceId));
    for (const t of counted) tradeCounts.set(t.marketId, (tradeCounts.get(t.marketId) ?? 0) + 1);
  }

  const byMetric = new Map<string, MetricsOverviewHorizon[]>();
  for (const m of openMarkets) {
    const list = byMetric.get(m.metricId) ?? [];
    list.push({
      marketId: m.id,
      targetDate: m.targetDate,
      settlesOn: periodEndInstant(m.targetDate).toISOString(),
      pool: (m.pool as number) ?? 0,
      trades: tradeCounts.get(m.id) ?? 0,
    });
    byMetric.set(m.metricId, list);
  }
  for (const list of byMetric.values()) list.sort((a, b) => a.settlesOn.localeCompare(b.settlesOn));

  return {
    defaultCredits: ws[0]?.newMarketLiquidityCredits ?? 0,
    autoFund: ws[0]?.autoFundNewMarkets ?? false,
    metrics: rows.map(row => ({
      id: row.id,
      name: row.name,
      description: row.description || '',
      rangeMin: AMM_DEFAULTS.rangeMin,
      rangeMax: row.marketRangeMax ?? AMM_DEFAULTS.rangeMax,
      credits: (row.liquidityCredits as number | null) ?? null,
      curve: (row.timePreference as TimePreference | null)?.enabled === true,
      horizons: byMetric.get(row.id) ?? [],
    })),
  };
}
