...
/** The three things an approved proposal can be
 *  (docs/guides/proposals.md, "After approval: say whether it happened"). */
export const DELIVERY_STATES = ['not_started', 'in_progress', 'delivered'] as const;
export type DeliveryState = (typeof DELIVERY_STATES)[number];
/** One line, not an essay: the evidence lives behind the link in it. */
export const DELIVERY_NOTE_MAX = 1000;

/**
 * Say whether the approved work happened.
 *
 * Only an approved proposal has a state to set: a declined one was never
 * promised and a pending one has nothing to report, so both are refused
 * rather than quietly carrying a field no surface shows. `delivered` stamps
 * the day and moving back off it clears the stamp, because a date that
 * outlives the state it recorded is a lie.
 *
 * An omitted note leaves the one already there; an empty one clears it. The
 * caller's right to set this is the workspace's `manage` capability, checked
 * on the route, exactly like approving.
 */
export async function setProposalDelivery(
  proposalId: string,
  workspaceId: string,
  update: { state: DeliveryState; note?: string | null },
  now: Date = new Date(),
): Promise<{ state: DeliveryState; note: string | null; deliveredAt: string | null }> {
  if (!(DELIVERY_STATES as readonly string[]).includes(update.state)) {
    throw new AppError(`state must be one of ${DELIVERY_STATES.join(', ')}`, 400);
  }
  if (typeof update.note === 'string' && update.note.length > DELIVERY_NOTE_MAX) {
    throw new AppError(`note must be ${DELIVERY_NOTE_MAX} characters or fewer`, 400);
  }
  ...
  if (proposal.status !== 'approved') {
    throw new AppError('Only an approved proposal has a delivery state', 400);
  }

  const note =
    update.note === undefined ? proposal.deliveryNote : update.note === null || update.note === '' ? null : update.note;
  const deliveredAt = update.state === 'delivered' ? (proposal.deliveredAt ?? now) : null;

  await db
    .update(proposals)
    .set({ deliveryState: update.state, deliveryNote: note, deliveredAt })
    .where(and(eq(proposals.id, proposalId), eq(proposals.workspaceId, workspaceId)));

  return { state: update.state, note, deliveredAt: deliveredAt ? deliveredAt.toISOString() : null };
}
