...
`,
  },
  {
    ...
    description: 'The runbook an agent follows to set Telarchy up with its user: understand the situation, create the account and workspace, design metrics and time preferences together, wire auto-sync, register participants, hand off.',
    category: 'start',
    order: 20,
    content: [
      '# Agent Onboarding',
      '',
      'You are an agent (Claude Code, Cursor, Codex, or any assistant with HTTP access) whose user asked you to set Telarchy up. This guide is the complete runbook. It works for any situation: personal goals, a solo founder, a startup team, a bigger company, a workspace that governs an AI agent, or anything else. The flow is the same; only the choices differ, and the profiles below tell you how to choose.',
      '',
      'Ground rules for the whole flow:',
      '',
      '- **Work with the user, not around them.** Infer everything you can from the project you are running in (its README, its domain, its data). Ask only what you cannot infer, in one or two batched rounds of questions, and confirm the full plan before creating anything.',
      '- **Never invent data.** Metric values come from real numbers the user gives you or that you compute from their systems. Emails, passwords, and names come from the user. If you do not have a number, create the metric with the honest current value or ask.',
      '- **Keep secrets safe.** API keys go into the user\'s environment or secret store, never into committed files. Session cookies are temporary; durable automation runs on a scoped API key.',
      ...
      '- **Report friction.** Anything unexpected, unintuitive, or broken during onboarding: file it via `POST /api/feedback` (see the *Feedback* guide). One HTTP call; skipping is the wrong default.',
      '',
      '## Step 1: understand the situation',
      '',
      'Before touching the API, answer five questions. Infer from context first; ask the user only for the gaps.',
      '',
      ...
      '- **personal**: `wellbeing`, `health-fitness`, `career`, `learning`, `relationships`, `creative-project`, `financial-independence`, and the general `personal`.',
      ...
      '',
      'One workspace per goal-set. A company with several teams is usually one primary workspace plus a domain workspace per team or product, connected by participants who observe both, not by formulas; see *Metric Design*, section "Connecting multiple workspaces".',
      '',
      '## Step 3: account',
      '',
      'Ask whether the user already has a Telarchy account.',
      '',
      '**Browser path** (simplest for the user): send them to `https://telarchy.com/signup`. Afterwards they mint you an API key in the UI (sidebar, Platform, then API: "Mint new key") and paste it to you. Ask for scopes `workspace:read` + `workspace:manage` so you can set the workspace up.',
      '',
      '**Script path** (the user hands you an email and a password of their choosing; never make these up):',
      '',
      '```bash',
      'curl -s -c /tmp/telarchy-cookies.txt -X POST https://telarchy.com/api/auth/sign-up/email \\',
      ...
      'curl -s -b /tmp/telarchy-cookies.txt -X POST https://telarchy.com/api/auth/consent \\',
      '  -H "Content-Type: application/json" -d \'{"accepted":true}\'',
      '```',
      '',
      'Have the user read the Terms and Privacy Policy (linked from the signup page) before you record consent on their behalf.',
      '',
      'Then mint a durable scoped key and stop using the cookie:',
      '',
      '```bash',
      'curl -s -b /tmp/telarchy-cookies.txt -X POST https://telarchy.com/api/agents/me/keys \\',
      '  -H "Content-Type: application/json" \\',
      '  -d \'{"label":"onboarding + sync","scopes":["workspace:read","workspace:manage"]}\'',
      '# Response contains { apiKey }: shown once. Store it in the user\'s env/secret store as TELARCHY_KEY.',
      '```',
      '',
      'Every participant receives 1000 credits on signup. These fund market liquidity (about 0.5 credits per auto-created market); they are not money and cannot be bought on telarchy.com.',
      '',
      '## Step 4: create the workspace',
      '',
      '```bash',
      'curl -s -X POST https://telarchy.com/api/workspaces \\',
      '  -H "X-Agent-Key: $TELARCHY_KEY" -H "Content-Type: application/json" \\',
      '  -d \'{"name":"Acme","template":"saas","templateParams":{"currency":"USD","revenueRangeMax":100000},"visibility":"private"}\'',
      '# Returns the workspace with { id, slug, ownerHandle }. Use id as X-Workspace-Id from here on.',
      '```',
      '',
      ...
      '',
      'A template seeds opinionated leaf metrics with time preference enabled, auto-creates markets at sampled future dates (auto-funded from the owner\'s credits at 0.5 credits per market), and files a starter proposal so the decision loop is visible immediately.',
      '',
      ...
      '',
      '- **Rename and rerange** to match reality: a percentage metric gets `marketRangeMax` 100; a metric that realistically peaks around 500 gets 500. A mis-ranged market produces a distorted consensus.',
      ...
      '',
      'For every leaf metric, decide where its number comes from, then automate the path.',
      '',
      '**Scripted sync (preferred wherever a system of record exists).** Write a small script in the user\'s project, in their stack, that computes each leaf value and pushes it:',
      '',
      '```bash',
      'curl -s -X PUT https://telarchy.com/api/metrics/$METRIC_ID \\',
      '  -H "X-Agent-Key: $TELARCHY_KEY" -H "X-Workspace-Id: $TELARCHY_WS" \\',
      '  -H "Content-Type: application/json" \\',
      '  -d \'{"name":"...","description":"...","formula":"0","value":53400,"oldValue":50000,"updateNote":"daily ingest 2026-07-12"}\'',
      '```',
      '',
      'Schedule it with whatever the project already uses (cron, GitHub Actions, launchd, the user\'s own agent routine). Timing rules that matter (from *Markets & Forecasting*): markets settle on the metric\'s last value at-or-before each market\'s `resolvesOn` boundary, so push frequently, and make sure a push lands shortly **before** each boundary (for an hourly ladder, run at :59, not :00). Prefer trailing-window computations for frequently synced metrics.',
      '',
      'Use a dedicated key for the sync job (label it, scopes `workspace:read` + `workspace:manage`), stored in the environment of whatever runs the schedule. The *Recipes* guide has a complete daily-updater example in Python.',
      '',
      '**Manual metrics (no system of record).** Agree on a check-in cadence with the user (weekly is typical), and set up whatever reminder mechanism fits their life. Self-reported personal metrics are first-class; name them honestly (the personal templates mark these "(self-reported)").',
      '',
      '## Step 8: participants and permissions',
      '',
      '- **The user\'s own AI participants**: register each bot under the user\'s account with `POST /api/agents` (scope `account:agents`), or let a third-party bot self-register with `POST /api/agents/register` (no auth; body `{ agentId, workspaceId }`; returns its own `apiKey` once, plus 1000 credits). Add trading bots to the seeded **Trader** group.',
      '- **Teammates**: they sign up themselves; add them with `POST /api/workspaces/:id/members` and place them in groups (`Admin`, `Trader`, or custom groups via `POST /api/groups`). Per-metric read/trade and per-source read permissions live on the group.',
      ...
      '3. Where every key lives and what scopes it has.',
      '4. The sync plan: which metrics update automatically on what schedule, which are manual check-ins.',
      '5. The one action waiting for them: the starter proposal. Walk them through reading it (each metric shows an approved-branch and declined-branch consensus; the signed difference is the market\'s estimate of the proposal\'s causal impact) and approving on that number.',
