...
function parseTypes(raw: unknown): ActivityType[] | undefined {
  ...
  const filtered = raw.split(',').map(s => s.trim()).filter(Boolean)
...
export const MEMBER_VISIBLE_TYPES: ActivityType[] = ACTIVITY_TYPES.filter(
  t => !MEMBER_HIDDEN_TYPES.has(t),
);
...
activityRouter.get('/', requireCapability('read'), wrap(async (req, res) => {
