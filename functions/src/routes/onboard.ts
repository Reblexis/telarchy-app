...
import {
  validateAgentId, normalizeBio, toUnits,
  SIGNUP_CREDITS, UNCLAIMED_SIGNUP_CREDITS,
} from '../lib/validation';
...
onboardRouter.post('/', wrap(async (req, res) => {
  ...
  if (existing) { res.status(409).json({ error: 'Participant id already taken' }); return; }
  ...
  const scopes = [
    'workspace:read', 'workspace:trade', 'workspace:manage',
    'account:read', 'account:write', 'account:keys', 'account:agents', 'account:feedback',
  ];
  ...
  await db.insert(agentApiKeys).values({
    hash: hashKey(rawKey), keyId, agentId: participantId,
    workspaceId: ws.id, label: 'onboarding', scopes,
  });
 ...
 */
onboardRouter.get('/claim/:token', wrap(async (req, res) => {
  const tokenHash = hashKey(req.params.token as string);
  const [agent] = await db.select({ id: agents.id, nickname: agents.nickname })
    .from(agents).where(eq(agents.claimTokenHash, tokenHash));
  if (!agent) { res.status(404).json({ error: 'Unknown or already-used claim token' }); return; }
  const wsRows = await db.select({ id: workspaces.id, name: workspaces.name, slug: workspaces.slug })
    .from(workspaces).where(eq(workspaces.createdBy, agent.id));
  res.json({
 ...
 */
onboardRouter.post('/claim', authMiddleware, wrap(async (req, res) => {
  const uid = req.auth?.uid;
  if (!uid) { res.status(401).json({ error: 'Sign in with a browser account to claim' }); return; }
  ...
  if (!target) { res.status(404).json({ error: 'Unknown or already-used claim token' }); return; }
  if (target.authUserId) { res.status(409).json({ error: 'Identity already claimed' }); return; }

  // The claiming account's own auto-provisioned participant, if any.
  const [own] = await db.select().from(agents)
  ...
  if (own && own.id !== target.id) {
    ...
    const [ownWs] = await db.select({ id: workspaces.id }).from(workspaces)
      .where(or(eq(workspaces.createdBy, own.id), eq(workspaces.createdBy, uid))).limit(1);
    if (activity || pos || ownWs) {
      res.status(409).json({
        error: 'This account already has an active participant identity. Claim from a fresh account, or keep using the API key.',
      });
      return;
    }
  }
  ...
  const wsRows = await db.select({ id: workspaces.id, name: workspaces.name, slug: workspaces.slug })
    .from(workspaces).where(eq(workspaces.createdBy, target.id));
