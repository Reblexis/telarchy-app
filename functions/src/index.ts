...
app.get('/api/help', (_req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      { method: 'POST', path: '/api/predictions/trade', auth: 'agent/admin', description: 'Trade on a market. Two modes: {marketId, direction: "higher"|"lower", amount} (bet direction), {marketId, value, amount} (system picks direction). Admin can add agentId to impersonate.' },
