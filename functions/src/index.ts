...
app.get('/api/help', (_req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      { method: 'POST', path: '/api/predictions/trade', auth: 'agent/admin', description: 'Trade on a market. Modes: {marketId, direction: "higher"|"lower", amount} (bet direction), {marketId, value, amount} (system picks direction), {marketId, targetValue, maxBudget} (buy shares until consensus reaches targetValue, spending at most maxBudget credits), {marketId, direction, sellShares} (sell shares). Admin can add agentId to impersonate.' },
