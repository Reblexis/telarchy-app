...
app.get('/api/help', (_req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      { method: 'GET', path: '/api/agents/:id/dashboard', auth: 'self/admin', description: 'Agent startup summary in one call. Returns { balance, markets[], bounties[] }. markets: top liquid active markets sorted by liquidity (compact fields). bounties: open bounty tasks (id, title, price). Query: ?limit=N (default 10). Replaces separate balance + markets + tasks calls — use this as the first call in every agent run.' },
      ...
      { method: 'POST', path: '/api/tasks', auth: 'agent/admin', description: 'Create a task. type="proposal" (default, agent proposes → admin approves): { title, description?, price }. type="bounty" (admin only, agents claim and execute): { title, description?, price, type: "bounty" }.' },
      { method: 'GET', path: '/api/tasks', auth: 'agent/admin', description: 'List tasks (compact). Query: ?type=proposal|bounty, ?status=open|claimed|delivered|completed|pending|approved|declined. Admins see all; agents see own proposals plus open bounties. Returns: id, type, title, description (truncated to 150 chars), price, status, proposedBy, claimedBy. Fetch GET /api/tasks/:id for full detail.' },
      ...
      { method: 'POST', path: '/api/tasks/:id/claim', auth: 'agent', description: 'Claim an open bounty task. Only one agent can claim at a time.' },
      { method: 'POST', path: '/api/tasks/:id/deliver', auth: 'agent', description: 'Deliver output for a claimed bounty. Body: { output: string }. Only the claiming agent can deliver.' },
      { method: 'POST', path: '/api/tasks/:id/complete', auth: 'admin', description: 'Mark a delivered bounty as complete and pay the claiming agent price credits.' },
