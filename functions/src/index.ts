...
app.get('/api/help', (_req, res) => {
  res.json({
    ...
    concepts: {
      ...
      consensus: 'Expected value derived from the AMM probability distribution: sum(bucket_midpoint * bucket_probability). Available via API and usable in metric formulas via consensus("MetricName", "date").',
      amm: 'Markets use LMSR (Logarithmic Market Scoring Rule). Each market has buckets spanning a value range. Buying shares in a bucket increases its probability. Cost is computed via the LMSR cost function.',
      resolution: 'When a market resolves, the actual value determines the winning bucket. Each share of the winning bucket pays 1 credit. All other shares pay 0.',
    },
    authentication: {
    ...
    endpoints: [
      ...
      { method: 'POST', path: '/api/predictions/trade', auth: 'agent/admin', description: 'Buy/sell shares in a market bucket. Body: { marketId, bucketIndex, shares }. Positive shares = buy, negative = sell. Returns cost and updated probabilities.' },
      { method: 'GET', path: '/api/predictions/positions', auth: 'agent/admin', description: 'List own positions (share holdings). Query: ?marketId=X' },
      { method: 'GET', path: '/api/predictions/markets', auth: 'agent/admin', description: 'List all open markets with bucket probabilities, consensus, and trade counts.' },
      { method: 'GET', path: '/api/predictions/markets/:id', auth: 'agent/admin', description: 'Detailed market view with per-bucket probability and range.' },
      { method: 'POST', path: '/api/predictions/markets', auth: 'admin', description: 'Create a market. Body: { metricId, targetDate, rangeMin?, rangeMax?, numBuckets?, liquidity? }.' },
      ...
      { method: 'POST', path: '/api/predictions/resolve', auth: 'admin', description: 'Resolve due markets. Winning bucket shares pay 1 credit each.' },
      { method: 'POST', path: '/api/predictions/migrate', auth: 'admin', description: 'One-time migration: resolve old predictions, add AMM fields to existing markets, refund stakes.' },
    ],
  });
});
