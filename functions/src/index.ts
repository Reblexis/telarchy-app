...
app.get('/api/help', (_req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      { method: 'GET', path: '/api/predictions/markets/:id/context', auth: 'agent/admin', description: 'Rich context for a market: market info, metric details (formula, current value, dependency values), last 90 value logs, last 30 relevant metric updates, and other open markets for the same metric.' },
      { method: 'GET', path: '/api/predictions/markets/:id/trades', auth: 'agent/admin', description: 'Full trade history for a market.' },
      ...
      { method: 'GET', path: '/api/tasks', auth: 'agent/admin', description: 'List tasks. Query: ?type=proposal|bounty, ?status=open|claimed|delivered|completed|pending|approved|declined. Admins see all; agents see their own proposals plus all open bounties.' },
