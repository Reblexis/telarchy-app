...
export const workspaces = pgTable('workspaces', {
   ...
   *  floor to outside proposals"): while true only a caller holding `manage`
   *  here may post one. False by default, so every floor stays open. */
  externalProposalsDisabled: boolean('external_proposals_disabled').notNull().default(false),
...
export const markets = pgTable(
  ...
  t => [
    ...
    // reaches settled books through resolved_at (migration 0131).
    ...
    // loadSeasonSettled; migration 0131).
...
export const positions = pgTable(
  ...
  t => [
    ...
    // A participant's own positions: profile, positions list (migration 0131).
...
export const trades = pgTable(
  ...
  t => [
    ...
    // A participant's own trades, newest first: the profile (migration 0131).
...
export const proposals = pgTable(
  'proposals',
  {
    ...
    resolvedAt: timestamp('resolved_at'),
    /**
     * The decision instant as the public actions log states it
     * (docs/data-room.md): the lapse (or, on rows from before lapsed_at, the
     * close) for a lapsed proposal, the close for a withdrawn one, the ruling
     * otherwise; null while pending or removed. Generated by Postgres from
     * the columns the decision paths already write, so no path can forget it.
     * It exists to be indexed: computed per row at read time, no index could
     * give a floor's decisions newest first, and a floor deciding a proposal a
     * second had its whole history sorted on every Live read. Migration 0130.
     */
    decidedAt: timestamp('decided_at').generatedAlwaysAs(
      sql`CASE WHEN status = 'lapsed' THEN COALESCE(lapsed_at, closed_at, resolved_at) WHEN status = 'withdrawn' THEN COALESCE(closed_at, resolved_at) WHEN status IN ('approved', 'declined', 'declined_spam') THEN resolved_at END`,
    ),
  ...
  t => [
    ...
    // The actions log's decision and delivery rows, newest first per floor.
    // Migration 0130.
    index('proposals_ws_decided_idx').on(t.workspaceId, t.decidedAt).where(sql`${t.decidedAt} IS NOT NULL`),
    index('proposals_ws_delivered_idx').on(t.workspaceId, t.deliveredAt).where(sql`${t.deliveredAt} IS NOT NULL`),
  ],
);
