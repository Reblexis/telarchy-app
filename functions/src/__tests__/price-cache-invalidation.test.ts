...
beforeEach(async () => {
  ...
  await db.insert(workspaces).values({
    id: WS, name: 'Cache', slug: 'cache', createdBy: 'trader', visibility: 'public',
  });
  await db.insert(metrics).values({
    id: METRIC, workspaceId: WS, name: 'M', value: 10, formula: '0', marketRangeMax: 100,
  });
  await db.insert(markets).values({
    id: MARKET, workspaceId: WS, metricId: METRIC, metricName: 'M',
    targetDate: '2026-12', rangeMin: 0, rangeMax: 100,
    shares: [0, 10], liquidity: B, pool: B * Math.LN2,
  ...
  await db.insert(trades).values({
    ...
    direction: 'higher', shares: 10, cost: 5, createdAt: T1,
  });
});

async function addTrade(id: string, extraShares: number, at: Date): Promise<void> {
  await db.insert(trades).values({
    id, workspaceId: WS, agentId: 'trader', marketId: MARKET,
    direction: 'higher', shares: extraShares, cost: 5, createdAt: at,
  });
  const [m] = await db.select().from(markets).where(eq(markets.id, MARKET));
  const shares = m.shares as [number, number];
  await db.update(markets).set({ shares: [shares[0], shares[1] + extraShares] })
