...
app.use((req, _res, next) => {
  (req as any).auth = {
    agentId: OWNER, uid: null, workspaceId: WS,
...
async function seed() {
  ...
  await provisionWorkspace(db as any, {
    wsId: WS, name: 'Freeze Test', createdBy: OWNER, ownerAgentId: OWNER, visibility: 'public',
  });
  await db.insert(metrics).values({
    id: METRIC, workspaceId: WS, name: 'Throughput', value: 0, formula: '0', marketRangeMax: 100,
  });
  await db.insert(markets).values([TRADED, UNTRADED].map((id, i) => ({
    id, workspaceId: WS, metricId: METRIC, metricName: 'Throughput',
    targetDate: `202${8 + i}`, rangeMin: 0, rangeMax: 100,
...
async function startSeason() {
  await db.insert(prizeSeasons).values({
    ...
    startsAt: new Date('2026-08-01'), endsAt: new Date('2026-12-31'),
    poolUsd: 1000, ladder: [{ place: 1, prizeUsd: 1000 }],
    workspaceIds: [WS], rulesUrl: 'https://telarchy.com/legal/season-1',
    status: 'running',
  });
}
...
describe('deleting a workspace inside a running season is refused', () => {
  ...
  test('a running season that does not name this workspace does not', async () => {
    ...
    await db.update(prizeSeasons).set({ workspaceIds: ['some-other-ws'] })
