import { computeContractors, jobImpact, type ContractorJob } from '../lib/contractors';
...
const pair = (approved: number | null, declined: number | null, metricId = HERO, targetDate = '2026-12-31') =>
  ({ metricId, targetDate, approvedConsensus: approved, declinedConsensus: declined });

const names = new Map<string, string | null>([['ana', 'Ana'], ['bo', 'Bo'], ['cy', null]]);
...
describe('computeContractors', () => {
  test('a job posted minutes ago counts as soon as the market prices it', () => {
    ...
    expect(rows[0]).toMatchObject({ id: 'ana', name: 'Ana', impact: 200, jobs: 1, pendingJobs: 1, pricedJobs: 1, earnedUsd: 0 });
  });
