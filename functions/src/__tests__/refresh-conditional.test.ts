...
import { workspaces, agents, metrics, markets, systemConfig, proposals } from '../db/schema';
...
describe('refreshRelativeDateMarkets and conditional markets', () => {
  ...
  test('re-spawns conditional markets for a pending proposal that has none left', async () => {
    ...
    await refreshRelativeDateMarkets(WS, { force: true });
    const baselines = await db.select().from(markets).where(and(
      eq(markets.workspaceId, WS), isNull(markets.proposalId),
    ));
    ...
    await db.insert(proposals).values({
      id: 'prop-dead', workspaceId: WS, proposedBy: OWNER,
      title: 'Pending proposal with dead conditionals', description: '',
      status: 'pending', conditionalMarketIds: ['dead-1', 'dead-2'],
      liquiditySubsidy: 0, subsidyContributions: {},
    });
    await db.insert(markets).values([
      {
        id: 'dead-1', workspaceId: WS, metricId: METRIC, metricName: 'Steam recent review percentage',
        proposalId: 'prop-dead', branch: 'approved', targetDate: baselines[0].targetDate,
        rangeMin: 0, rangeMax: 100, shares: [0, 0], liquidity: 0, pool: 0,
        active: false, resolved: true, voided: true,
      },
    ]);

    const before = await db.select().from(markets).where(and(
      eq(markets.workspaceId, WS), eq(markets.proposalId, 'prop-dead'), eq(markets.resolved, false),
    ));
    ...
    const live = await db.select().from(markets).where(and(
      eq(markets.workspaceId, WS), eq(markets.proposalId, 'prop-dead'), eq(markets.resolved, false),
    ));
  ...
  test('does not re-spawn conditional markets for a non-pending proposal', async () => {
    ...
    await db.insert(proposals).values({
      id: 'prop-approved', workspaceId: WS, proposedBy: OWNER,
      title: 'Approved proposal', description: '',
      status: 'approved', conditionalMarketIds: ['gone-1'],
      liquiditySubsidy: 0, subsidyContributions: {},
    });
    ...
    const live = await db.select().from(markets).where(and(
      eq(markets.workspaceId, WS), eq(markets.proposalId, 'prop-approved'), eq(markets.resolved, false),
    ));
