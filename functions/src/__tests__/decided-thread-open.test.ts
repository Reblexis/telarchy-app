...
async function seed(status: string) {
  ...
  await db.insert(workspaces).values({ id: WS, name: 'Decided Thread WS', createdBy: 'agent-dt', visibility: 'public' });
  await db.insert(proposals).values({
    id: `prop-${status}`, workspaceId: WS, proposedBy: 'agent-dt',
    title: 'Rewrite the store page', status,
  });
}

function post(proposalId: string, content: string) {
  return request(app).post(`/api/proposals/${proposalId}/messages`)
    .set('X-Test-Agent-Id', 'agent-dt').set('X-Workspace-Id', WS)
    .set('Content-Type', 'application/json').send({ content });
}
...
describe('a decided contract keeps its thread open', () => {
  test.each(['approved', 'declined'])('posting on a %s proposal lands', async (status) => {
    ...
    const read = await request(app).get(`/api/proposals/prop-${status}/messages`)
      .set('X-Test-Agent-Id', 'agent-dt').set('X-Workspace-Id', WS);
